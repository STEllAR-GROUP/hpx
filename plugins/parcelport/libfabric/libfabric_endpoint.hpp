...
#ifndef HPX_PARCELSET_POLICIES_LIBFABRIC_ENDPOINT_HPP
#define HPX_PARCELSET_POLICIES_LIBFABRIC_ENDPOINT_HPP
...
/*
#include <plugins/parcelport/libfabric/rdma/libfabric_event_channel.hpp>
#include <plugins/parcelport/libfabric/rdma/libfabric_shared_receive_queue.hpp>
#include <plugins/parcelport/libfabric/rdma/libfabric_sender_receiver.hpp>
#include <plugins/parcelport/libfabric/rdma/libfabric_completion_queue.hpp>
#include <plugins/parcelport/libfabric/rdma/libfabric_memory_region.hpp>
#include <plugins/parcelport/libfabric/rdma/libfabric_protection_domain.hpp>
*/
//
#include <inttypes.h>
#include <stdexcept>
...
#define HPX_PARCELPORT_LIBFABRIC_MAX_WORK_REQUESTS 1024
...
/*
        ...
        libfabric_endpoint(
            ...
            : libfabric_sender_receiver(nullptr)
         {
            LOG_DEVEL_MSG("libfabric_endpoint Listening Server Constructor");
            //
            init();
            create_cm_id();
            bind(local_address);
        ...
        libfabric_endpoint(
            ...
            libfabric_protection_domain_ptr domain,
            libfabric_completion_queue_ptr CompletionQ,
            rdma_memory_pool_ptr pool,
            libfabric_shared_receive_queue_ptr SRQ,
            libfabric_event_channel_ptr event_channel) :
                libfabric_sender_receiver(nullptr)
        {
            LOG_DEVEL_MSG("libfabric_endpoint receive connection constructor");
            //
            ...
            create_queue_pair(domain_, completion_queue_, completion_queue_,
                HPX_PARCELPORT_LIBFABRIC_MAX_WORK_REQUESTS, false);
        }
        ...
        libfabric_endpoint(
            ...
            libfabric_event_channel_ptr event_channel) :
                libfabric_sender_receiver(nullptr)
        {
            LOG_DEVEL_MSG("libfabric_endpoint create connection constructor "
        ...
        // ---------------------------------------------------------------------------
        libfabric_completion_queue_ptr& get_completion_queue(void) {
        ...
        // ---------------------------------------------------------------------------
        inline libfabric_memory_region *get_free_region(size_t size)
        {
            libfabric_memory_region* region = this->memory_pool_->allocate_region(size);
            if (!region) {
        ...
        {
            return event_channel_->poll_libfabric_event_channel(
                [this, &f]()
                {
                    struct rdma_cm_event *cm_event;
                    int err = event_channel_->get_event(libfabric_event_channel::no_ack_event,
                        nullptr, cm_event);
                    if (err != 0) return 0;
                    return f(cm_event);
                }
        ...
        // ---------------------------------------------------------------------------
        int get_event(libfabric_event_channel::event_ack_type ack,
            ...
            std::chrono::milliseconds timeout)
        {
            ...
            if (rc != 0) {
                fabric_error e(errno, "rdma_resolve_addr() failed");
                LOG_ERROR_MSG("error resolving remote address "
                    << sockaddress(localAddr) << ": "
                    << sockaddress(remoteAddr) << ": "
                    << fabric_error::error_string(e.error_code()));
                throw e;
            }
        ...
        int handle_addr_resolved(struct rdma_cm_event *event)
        {
            ...
            if (state_!=connection_state::resolving_address) {
                fabric_error e(0, "invalid state in resolving_address");
                std::terminate();
                return -1;
            }
            libfabric_event_channel::ack_event(event);
        ...
        int resolve_route(void)
        {
            if (state_!=connection_state::address_resolved) {
                fabric_error e(0, LOG_FORMAT_MSG("invalid state in resolve_route"));
                std::terminate();
                throw e;
            }
            ...
            if (rc != 0) {
                fabric_error e(rc, LOG_FORMAT_MSG("error resolving route to "
        ...
        int handle_route_resolved(struct rdma_cm_event *event)
        {
            ...
            if (state_!=connection_state::resolving_route) {
                fabric_error e(0, "invalid state in handle_route_resolved");
                std::terminate();
                return -1;
            }
        ...
        int abort()
        {
                ...
                state_==connection_state::terminated)
            {
                ...
                fabric_error e(0, "invalid state in abort");
                std::terminate();
                return -1;
            }
        ...
        int accept()
        {
            ...
            if (rc != 0) {
                int err = errno;
                LOG_ERROR_MSG(
                    "error accepting connection: " << fabric_error::error_string(err));
                return err;
            }
        ...
        int reject(rdma_cm_id *id)
        {
            ...
            if (err != 0) {
                LOG_ERROR_MSG("error rejecting connection on cmid "
                    << hexpointer(id) << fabric_error::error_string(errno));
                return -1;
            }
        ...
        int handle_establish(struct rdma_cm_event *event)
        {
            ...
            if (event->event == RDMA_CM_EVENT_ESTABLISHED) {
                if (state_!=connection_state::connecting &&
                    state_!=connection_state::accepting)
                {
                    fabric_error e(0, "invalid state in handle_establish for "
        ...
        int disconnect()
        {
            ...
            if (state_!=connection_state::connected &&
                ...
                state_!=connection_state::terminated)
            {
                fabric_error e(0, LOG_FORMAT_MSG("invalid state in disconnect "));
                std::terminate();
                throw e;
            }
            ...
            if (err != 0) {
                err = abs(err);
                LOG_ERROR_MSG(
                    "error disconnect: " << fabric_error::error_string(err));
                return err;
            }
        ...
        int handle_disconnect(struct rdma_cm_event *event)
        {
            ...
            if (state_!=connection_state::disconnecting &&
                ...
                state_!=connection_state::connected)
            {
                fabric_error e(0, "invalid state in handle_disconnect");
                std::terminate();
                return -1;
            }
        ...
        // ---------------------------------------------------------------------------
        int create_srq(libfabric_protection_domain_ptr domain)
        {
            try {
                srq_ = std::make_shared<libfabric_shared_receive_queue>(domain);
            }
            catch (...) {
                return 0;
            }
        ...
        // ---------------------------------------------------------------------------
        inline libfabric_shared_receive_queue_ptr SRQ() {
            return srq_;
        }
        ...
        // ---------------------------------------------------------------------------
        libfabric_event_channel_ptr get_event_channel(void) const {
    ...
    protected:
        ...
        void init(void)
        {
            ...
            if (!event_channel_) {
                event_channel_ = std::make_shared<libfabric_event_channel>();
            }
            clear_counters();
        ...
        void create_cm_id(void)
        {
            ...
            if (!event_channel_->get_libfabric_event_channel()) {
            ...
            int err = rdma_create_id(
                event_channel_->get_libfabric_event_channel(), &cmId_, this, RDMA_PS_TCP);
            if (err != 0) {
                fabric_error e(err, "rdma_create_id() failed");
                LOG_ERROR_MSG(
                    "error creating rdma cm id: " << fabric_error::error_string(err));
                throw e;
            }
        ...
        // ---------------------------------------------------------------------------
        void create_queue_pair(libfabric_protection_domain_ptr domain,
            libfabric_completion_queue_ptr sendCompletionQ,
            libfabric_completion_queue_ptr recvCompletionQ,
            uint32_t maxWorkRequests, bool signalSendQueue)
        {
            ...
            if (rc != 0) {
                fabric_error e(errno, "rdma_create_qp() failed");
                LOG_ERROR_MSG(
        ...
        libfabric_shared_receive_queue_ptr srq_;

        // Event channel for notification of RDMA connection management events.
        libfabric_event_channel_ptr event_channel_;
        ...
        libfabric_protection_domain_ptr domain_;
...
*/
    ...
    // Smart pointer for libfabric_endpoint object.
    typedef std::shared_ptr<libfabric_endpoint> libfabric_endpoint_ptr;
