...
public:
    ...
    verbs_device(std::string device, std::string interface) : context_(NULL)
    {
        ...
        for (int index = 0; index < numDevices; ++index) {
            LOG_DEBUG_MSG("checking device " << ibv_get_device_name(devices_[index]) << " against " << device.c_str());
        ...
        if (!interface.empty()) {
            ...
            for (interface_ = interfaces_; interface_ != NULL; interface_ = interface_->ifa_next) {
                //      std::cout << "checking interface " << interface_->ifa_name << " against " << interface.c_str() << std::endl;
                if ((strcmp(interface_->ifa_name, interface.c_str()) == 0) && (interface_->ifa_addr->sa_family == AF_INET)) {
                    break;
                }
            }
...
//                << setw(20) << "sys_image_guid " << guid_str(device_attr.sys_image_guid, buf) << "\n"
            ...
            if (verbose) {
                info << setw(20) << "max_mr_size " <<
                    ...
                    << setw(20) << "device_cap_flags " << device_attr.device_cap_flags << "\n"
                    ...
                    << setw(20) << "max_qp_rd_atom " << device_attr.max_qp_rd_atom << "\n"
                    << setw(20) << "max_ee_rd_atom " << device_attr.max_ee_rd_atom << "\n"
                    << setw(20) << "max_res_rd_atom " << device_attr.max_res_rd_atom << "\n"
                    << setw(20) << "max_qp_init_rd_atom " << device_attr.max_qp_init_rd_atom << "\n"
                    << setw(20) << "max_ee_init_rd_atom " << device_attr.max_ee_init_rd_atom << "\n"
//                    << setw(20) << "atomic_cap " << atomic_cap_str(device_attr.atomic_cap) << "\n"
                    << setw(20) << "max_ee " << device_attr.max_ee << "\n"
                    << setw(20) << "max_rdd " << device_attr.max_rdd << "\n"
                    << setw(20) << "max_mw " << device_attr.max_mw << "\n"
                    << setw(20) << "max_raw_ipv6_qp " << device_attr.max_raw_ipv6_qp << "\n"
                    << setw(20) << "max_raw_ethy_qp " << device_attr.max_raw_ethy_qp << "\n"
                    << setw(20) << "max_mcast_grp " << device_attr.max_mcast_grp << "\n"
                    << setw(20) << "max_mcast_qp_attach " << device_attr.max_mcast_qp_attach << "\n"
                    << setw(20) << "max_total_mcast_qp_attach " << device_attr.max_total_mcast_qp_attach << "\n"
                    << setw(20) << "max_ah " << device_attr.max_ah << "\n"
                    << setw(20) << "max_fmr " <<  device_attr.max_fmr << "\n";
                if (device_attr.max_fmr) {
                    info << setw(20) << "max_map_per_fmr " << device_attr.max_map_per_fmr << "\n";
                }
                info << setw(20) << "max_srq " << device_attr.max_srq << "\n";
                if (device_attr.max_srq) {
                    info << setw(20) << "max_srq_wr " << device_attr.max_srq_wr << "\n"
                        << setw(20) << "max_srq_sge " << device_attr.max_srq_sge << "\n";
                }
                info << setw(20) << "max_pkeys " << device_attr.max_pkeys << "\n"
                    << setw(20) << "local_ca_ack_delay " << device_attr.local_ca_ack_delay << "\n";
            }
        }
        return info.str();
    }
