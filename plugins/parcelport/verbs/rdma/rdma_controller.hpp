...
namespace verbs
{
    ...
    public:
        ...
        bool isTerminated() { return (clients_.size()==0); }

        typedef std::function<int(struct sockaddr_in *, struct sockaddr_in *)>
        ConnectRequestFunction;
        typedef std::function<int(std::pair<uint32_t,uint64_t>, verbs_endpoint_ptr)>
        ConnectionFunction;
        ...
        verbs_completion_queue *get_shared_receive_queue() { return nullptr; }
        verbs_endpoint *get_client_from_completion(struct ibv_wc &completion) {
            return nullptr;
        }

        // Listener for RDMA connections.
        verbs_endpoint_ptr getServer() { return this->server_endpoint_; }
        verbs_protection_domain_ptr getProtectionDomain() { return this->protection_domain_; }
        verbs_endpoint* getClient(uint32_t qp) {
            return clients_[qp].get();
        }

        rdma_memory_pool_ptr getMemoryPool() { return memory_pool_; }
        ...
        typedef std::function<int(struct ibv_wc completion, verbs_endpoint *client)> CompletionFunction;
        void setCompletionFunction(CompletionFunction f) { this->completion_function_ = f;}

        int num_clients() { return clients_.size(); }
