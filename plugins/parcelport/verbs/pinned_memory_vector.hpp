...
class pinned_memory_vector
{
  public:
    ...
    pinned_memory_vector(T* p, std::size_t s, deleter_callback cb, allocator_type* alloc, rdma_memory_region *r) :
    ...
    std::size_t capacity() { return m_region_ ? m_region_->get_size() : 0; }
    ...
    inline void resize(std::size_t s) {
        ...
        if (m_region_) {
            if (s>m_region_->get_size()) {
                LOG_ERROR_MSG("Resizing from " << m_region_->get_size() << " to " << s)
