...
inline void
producer(std::size_t count, std::size_t size, boost::lockfree::fifo<void*>* bin)
{
    for (register std::size_t i = 0; i != count; ++i)
        bin->enqueue(malloc(size));
}
...
int main(int argc, char** argv)
{
    ...
    bins = reinterpret_cast<fifo<void*>*>   
        (::calloc(threads, sizeof(fifo<void*>)));

    for (std::size_t i = 0; i != threads; ++i)
        new (&bins[i]) fifo<void*>(count); 
