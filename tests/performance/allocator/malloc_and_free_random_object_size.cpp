...
inline void
producer(std::vector<boost::uint32_t>* size, boost::lockfree::fifo<void*>* bin)
{
    const std::size_t elements = size->size();

    for (register std::size_t i = 0; i != elements; ++i)
        bin->enqueue(malloc((*size)[i]));
}

inline void consumer(boost::lockfree::fifo<void*>* bin)
{
    void* value = 0;
...
int main(int argc, char** argv)
{
    ...
    fifo<void*>* bins = reinterpret_cast<fifo<void*>*>   
        (::calloc(threads, sizeof(fifo<void*>)));
    ...
    for (std::size_t i = 0; i != threads; ++i)
    {
        ...
        new (&bins[i]) fifo<void*>(sizes[i].size()); 
    }
