...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        {
            resolver.bind(gid_type(a), address(here, 1, a));
        }
    ...
    // unbind the address
        ...
        std::cout << "Measure_Unregister:"<< total_time / MAX_ITERATIONS << std::endl << std::flush;

        resolver.get_statistics_mean(timings);
        std::cout << " Time taken by unregister is: " << timings[10] <<  std::endl <<std::flush;
