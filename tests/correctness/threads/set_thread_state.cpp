    ...
    )
{
    ...
    std::cout << "tree_boot: count(" << count << "), actors("
    ...
    //wait(promises);
    for (boost::uint64_t i = 0; i < promises.size(); ++i)
        promises[i].get();
}
...
int hpx_main(variables_map& vm)
{
...

    {
        ...
        boost::uint64_t thread = boost::uint64_t(register_thread_nullary
            (boost::function<void()>(test_dummy_thread)));
...
int main(int argc, char* argv[])
{
    ...
    cmdline.add_options()
        ( "futures"
        , value<boost::uint64_t>()->default_value(0x1000)
