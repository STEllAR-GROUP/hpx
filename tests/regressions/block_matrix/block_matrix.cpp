...
block_vector_t::block_vector_t(
    std::shared_ptr<structure_t> str, IL<P<int, IL<double>>> x)
  : block_vector_t(str)
{
  ...
  , elts(istr->B * jstr->B)
{
    for (std::ptrdiff_t jb = 0; jb < jstr->B; ++jb)
    {
        ...
        {
            elts[ib + istr->B * jb] = hpx::new_<matrix_t_client>(
                istr->locs[ib], istr->size(ib), jstr->size(jb));
        }
    }
}

block_matrix_t::block_matrix_t(std::shared_ptr<structure_t> istr,
    std::shared_ptr<structure_t> jstr, IL<IL<P<P<int, int>, IL<IL<double>>>>> a)
