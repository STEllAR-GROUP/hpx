...
hpx::future<std::uint64_t> fibonacci(std::uint64_t n)
{
    ...
    if (n < threshold) return hpx::make_ready_future(
        hpx::async( hpx::util::annotated_function("fibonacci_serial",
        &fibonacci_serial), n));

    hpx::future<std::uint64_t> lhs_future =
        hpx::async(hpx::util::annotated_function("lhs", &fibonacci), n-1);
    hpx::future<std::uint64_t> rhs_future =
        hpx::async(hpx::util::annotated_function("rhs", &fibonacci), n-2);
