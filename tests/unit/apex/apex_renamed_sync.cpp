...
{
    hpx::util::rename_function t(name.c_str());
...
int hpx_main(variables_map& vm)
{
    ...
    {
        hpx::util::rename_function annotate("launch outer");
        for (int i=0; i<np_total; ++i)
        {
            // normal priority
            auto f3 = hpx::async(NP_executor,
            &dummy_task,
                "NP task", np_m).then(hpx::launch::sync,
                ([&](auto &&){
                    counter--;
                    if (++counter1 % np_loop == 0) {
                        hpx::util::rename_function annotate("launch inner");
