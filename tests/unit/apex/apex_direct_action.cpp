...
// invocation to work.
HPX_PLAIN_DIRECT_ACTION(fibonacci, fibonacci_action);
...
std::uint64_t fibonacci(std::uint64_t n)
{
    hpx::util::rename_function(__func__);
    ++count;
...
int main(int argc, char* argv[])
{
    ...
    apex_profile * prof = apex::get_profile("fibonacci");
    HPX_TEST(prof != 0);
    ...
    std::cout << "APEX measured calls to fibonacci: "
        << prof->calls << std::endl;
    HPX_TEST_EQ(1ULL, static_cast<std::uint64_t>(prof->calls));
