...
void test_for_each_prefetching()
{
    ...
    test_for_each_prefetching(execution_policy(par), IteratorTag());
    test_for_each_prefetching(execution_policy(par_vec), IteratorTag());
    test_for_each_prefetching(execution_policy(par(task)), IteratorTag());
#endif
}
...
#if defined(HPX_HAVE_GENERIC_EXECUTION_POLICY)
    test_for_each_prefetching_exception(execution_policy(par), IteratorTag());
    test_for_each_prefetching_exception(execution_policy(par(task)), IteratorTag());
#endif
}
...
#if defined(HPX_HAVE_GENERIC_EXECUTION_POLICY)
    test_for_each_prefetching_bad_alloc(execution_policy(par), IteratorTag());
    test_for_each_prefetching_bad_alloc(execution_policy(par(task)), IteratorTag());
#endif
}
