...
void test_count_if(ExPolicy const& policy, IteratorTag)
{
    ...
    std::size_t num_items = hpx::parallel::count_if(policy,
        iterator(boost::begin(c)), iterator(boost::end(c)), [](std::size_t x){ return x<50;});
...
void test_count_if_exception()
{
    ...
    test_count_if_exception(vec, IteratorTag());
    ...
    test_count_if_exception(execution_policy(vec), IteratorTag());
...
void test_count_if_bad_alloc(ExPolicy const& policy, IteratorTag)
{
    ...
    try {
        std::size_t res = hpx::parallel::count_if(policy,
            decorated_iterator(
                boost::begin(c),
                [](){ throw std::bad_alloc(); }),
            decorated_iterator(boost::end(c), [](){}),
...
void test_count_if_bad_alloc()
{
    ...
    test_count_if_bad_alloc(vec, IteratorTag());
    ...
    test_count_if_bad_alloc(execution_policy(vec), IteratorTag());
