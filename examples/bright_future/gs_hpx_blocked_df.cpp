        ...
        {
            {
                ...
                for(size_type y_block = 0, y = 0; y_block < n_y; y_block += block_size, ++y)
                {
                    for(size_type x_block = 0, x = 0; x_block < n_x; x_block += block_size, ++x)
                    {
                        ...
                        dataflow_base<void> deps  = dataflow<dependency_action>(remote_id);
                        if(iter==0)
                        {
                            deps = dataflow<dependency_action>(remote_id, deps, init_rhs_promises(x, y), prev(x,y));
                        ...
                        deps = dataflow<dependency_action>(remote_id, deps, prev(x,y));
