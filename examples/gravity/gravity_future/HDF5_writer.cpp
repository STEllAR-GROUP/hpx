...
void printval (future<void> const & mp,config_f& param,uint64_t k,uint64_t t,
 ...
 wait(mp);
 if (t%param.print==0) {
  uint64_t width=log10(param.steps)+1; //number of placeholders in filename
  string a=boost::lexical_cast<string>(t); //turn int timestep into a string
  //set number of placeholders using boost::format
  string frm=boost::str(boost::format("%%s_%%0%dd.h5") %width);
  //instert the rest of the file name in to the boost string "name"
  string name=boost::str(boost::format(frm) %param.output %a);
  const H5std_string FILE_NAME(name); //Define file name
  const H5std_string DATASET_NAME("Coordinates"); //Define dataset name
  H5File file(FILE_NAME, H5F_ACC_TRUNC); //create HDF5 file
  hsize_t fdim[]={k,11}; //define the file dimentions
  DataSpace fspace(2,fdim); //create the dataspace
  DataSet dataset=file.createDataSet(DATASET_NAME, PredType::NATIVE_DOUBLE,
                                     fspace); //create the dataset
  //write from pts_timestep to the the dataset
  dataset.write(&pts_timestep[t][0],PredType::NATIVE_DOUBLE);
  dataset.close(); //close the dataset
  file.close(); //close the file
 }
 if (debug) { //write to the text files
  uint64_t tn=t+1;
