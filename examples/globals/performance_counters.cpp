...
///////////////////////////////////////////////////////////////////////////////
void shutdown_(manage_counter_type* type_, manage_counter* instance_)
{
    BOOST_ASSERT(type_); 
...
void startup_()
{
    manage_counter_type* type_ = new manage_counter_type;
    manage_counter* instance_ = new manage_counter;

    // Install the counters.
    type_->install("/foo/buzz", counter_raw);  
    instance_->install("/foo(bar)/buzz", bar); 

    // Register the shutdown function which will free these counters.
