...
! diagnosis array
  ...
  implicit none
  interface ! {{{
    subroutine err_check(f1,f2,numberpe,stdout)
      !use global_parameters
      use precision
      implicit none
!     global parameters
      real(kind=wp) f1, f2, FOM, expected
      integer numberpe, ierr,stdout
...
!hjw          write(ihistory,102)amp_mode(k,j,i)
     ...
     if(istep>=mstep)then
        call err_check(fdum(1),fdum(19),numberpe,stdout)
     endif
...
subroutine test_diagnosis(hpx4_bti,&
                          zion0, nparam,mimax)
  use, intrinsic :: iso_c_binding, only : c_ptr
  use precision
  TYPE(C_PTR), INTENT(IN), VALUE :: hpx4_bti
...
!  global parameters
  ...
  integer mimax
  ! diagnosis thread safety
  real(kind=wp) zion0(nparam,mimax)

  print*,' INSIDE TEST DIAGNOSIS ', zion0(5,1)
end subroutine test_diagnosis

subroutine diagnosis_future(hpx4_bti,&
                     xnormal,&
! global parameters
                       ihistory,&
          mpsi,mthetamax,mzeta,mzetamax,&
...
!  global parameters
  integer :: ihistory
  integer nparam,mimax,memax,mgrid
  integer mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,mstep,mstepall,stdout,mype,numberpe,&
       nbound,irun,&
       nhybrid
  real(kind=wp) a0,a1,a,q0,q1,q2,kappati,&
       gyroradius,&
       tite,rc,rw,qion,qelectron,aion,aelectron
  integer mtheta(0:mpsi)

! particle array
  real(kind=wp) zion(nparam,mimax)
  real(kind=wp) zion0(nparam,mimax)
  real(kind=wp) zelectron(6,memax)
  real(kind=wp) zelectron0(6,memax)

! particle decomp
  integer  :: myrank_partd

! field array
  integer igrid(0:mpsi)
  real(kind=wp) gradt(mpsi)
  real(kind=wp) phi(0:mzeta,mgrid)
  real(kind=wp) Total_field_energy(3)
...
end subroutine diagnosis_future
