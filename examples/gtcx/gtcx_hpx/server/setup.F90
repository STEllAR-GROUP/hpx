...
!========================================================================
  ...
  implicit none
  interface ! {{{
    subroutine read_input_params(hpx4_bti,micell,mecell,r0,b0,temperature,edensity0,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,msnap,mstep,mstepall,stdout,mype,numberpe,&
...
! diagnosis array
   ...
   subroutine set_particle_decomp(hpx4_bti, &
             t_gids, p_gids,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
...
! Read the input file that contains the run parameters
  call read_input_params(hpx4_bti,micell,mecell,r0,b0,temperature,edensity0,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,msnap,mstep,mstepall,stdout,mype,numberpe,&
...
! Set up the particle decomposition within each toroidal domain
  call set_particle_decomp(hpx4_bti, &
             t_gids, p_gids,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
...
!=============================================================================
  ...
  implicit none
  interface
    subroutine broadcast_input_params(hpx4_bti,micell,mecell,r0,b0,temperature,edensity0,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,msnap,mstep,mstepall,stdout,mype,numberpe,&
...
! Now send the parameter values to all the other MPI processes
  call broadcast_input_params(hpx4_bti,micell,mecell,r0,b0,temperature,edensity0,&
! global parameters
                       ihistory,snapout,maxmpsi,&
          mi,mimax,me,me1,memax,mgrid,mpsi,mthetamax,mzeta,mzetamax,&
       istep,ndiag,ntracer,msnap,mstep,mstepall,stdout,mype,numberpe,&
...
! other variables
  ...
  integer,parameter :: n_integers=20+2*temp_num_mode,n_reals=28
...
! Send input parameters to all processes
  ...
  call broadcast_parameters_cmm(hpx4_bti,integer_params,real_params,&
                                n_integers,n_reals);
...
! particle decomp
  ...
  integer count_t, count_p,t_rank,p_rank
...
!!!!!!  call mpi_comm_rank(toroidal_comm,myrank_toroidal,ierror)
  ...
  allocate(t_gids(ntoroidal),p_gids(npartdom),STAT=mtest)
  if(mtest /= 0) then
    write(0,*)mype,' problem allocating toroidal or particle communicator mtest= ',mtest
  end if
  ...
  do i=0,numberpe-1
    ...
    if ( p_rank .eq. myrank_partd ) then
      if ( count_t .gt. ntoroidal ) then
        write(0,*)mype, ' problem figuring out the toroidal communicator '
      else
        ! this gives the global thread identifier 
        ! of all threads who share the same myrank_partd
        t_gids(count_t) = i
        count_t = count_t + 1
      end if
    end if
    if ( t_rank .eq. myrank_toroidal ) then
      if ( count_p .gt. npartdom ) then
        write(0,*)mype, ' problem figuring out the partd communicator '
      else
        ! this gives the global thread identifier 
        ! of all threads who share the same myrank_toroidal
        p_gids(count_p) = i
        count_p = count_p + 1
      end if
    end if
