...
fibonacci4_phased_action;

typedef 
    actions::plain_result_action2<int, int, int, fib_rhs> 
fibonacci4_phased_rhs_action;


typedef lcos::eager_future<fibonacci4_phased_action> fibonacci_future;
typedef lcos::eager_future<fibonacci4_phased_rhs_action> fibonacci_rhs_future;

HPX_REGISTER_PLAIN_ACTION(fibonacci4_phased_action);
HPX_REGISTER_PLAIN_ACTION(fibonacci4_phased_rhs_action);
...
int hpx_main(boost::program_options::variables_map &vm)
{
    ...
    argument = boost::lexical_cast<int>(
        rt.get_config().get_entry("application.fibonacci4_phased.argument", argument));
...
bool parse_commandline(int argc, char *argv[], po::variables_map& vm)
{
    ...
    try {
        po::options_description desc_cmdline ("Usage: fibonacci4_phased [options]");
        ...
        std::cerr << "fibonacci4_phased: exception caught: " << e.what() << std::endl;
        return false;
    }
    return true;
}
...
split_ip_address(std::string const& v, std::string& addr, boost::uint16_t& port)
{
    ...
    try {
        ...
        std::cerr << "fibonacci4_phased: illegal port number given: " << v.substr(p+1) << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        std::cerr << "fibonacci4_phased: std::exception caught: " << e.what() << "\n";
        return -1;
    }
    catch (...) {
        std::cerr << "fibonacci4_phased: unexpected exception caught\n";
        return -2;
    }
    return 0;
}
