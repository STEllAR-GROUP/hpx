...
namespace easy_bmp {
    ...
    RGBApixel BMP::GetPixel(int i, int j) const
    {
        ...
        {
            cout
                << "EasyBMP Warning: Attempted to access non-existent pixel;"
                << endl
    ...
    bool BMP::SetColor(int ColorNumber, RGBApixel NewColor)
    {
        ...
        if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: Attempted to change color table for "
                        "a BMP object"
                     << endl
        ...
        if (!Colors)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: Attempted to set a color, but the "
                        "color table"
                     << endl
    ...
    RGBApixel BMP::GetColor(int ColorNumber) const
    {
        ...
        if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: Attempted to access color table for "
                        "a BMP object"
                     << endl
                     << "                 that lacks a color table. Ignoring "
                        "request."
        ...
        if (!Colors)
        {
            ...
            {
                cout
                    << "EasyBMP Warning: Requested a color, but the color table"
                    << endl
                    << "                 is not defined. Ignoring request."
                    << endl;
            }
    ...
    bool BMP::SetBitDepth(int NewDepth)
    {
            ...
            NewDepth != 24 && NewDepth != 32)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: User attempted to set unsupported "
                        "bit depth "
                     << NewDepth << "." << endl
    ...
    bool BMP::WriteToFile(const char* FileName)
    {
        ...
        if (!EasyBMPcheckDataSize())
        {
            ...
            {
                cout
                    << "EasyBMP Error: Data types are wrong size!" << endl
        ...
        if (BitDepth != 16)
        {
            ...
            while (j > -1)
            {
                ...
                if (!Success)
                {
                    if (EasyBMPwarnings)
                    {
                        cout << "EasyBMP Error: Could not write proper amount "
                                "of data."
                             << endl;
                    }
                    j = -1;
    ...
    bool BMP::ReadFromFile(const char* FileName)
    {
        ...
        NotCorrupted &= SafeFread(
            (char*) &(bmih.biXPelsPerMeter), sizeof(ebmpDWORD), 1, fp);
        NotCorrupted &= SafeFread(
            (char*) &(bmih.biYPelsPerMeter), sizeof(ebmpDWORD), 1, fp);
        NotCorrupted &=
        ...
        if (bmih.biCompression > 3)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Error: " << FileName
                     ...
                     << "               The file is probably an old OS2 bitmap "
                        "or corrupted."
                     << endl;
            }
            SetSize(1, 1);
        ...
        if (BitDepth != 16)
        {
            ...
            while (j > -1)
            {
                ...
                if (BytesRead < BufferSize)
                {
                    ...
                    if (EasyBMPwarnings)
                    {
                        cout << "EasyBMP Error: Could not read proper amount "
                                "of data."
                             << endl;
                    }
                }
                else
                {
                    ...
                    if (!Success)
                    {
                        if (EasyBMPwarnings)
                        {
                            cout << "EasyBMP Error: Could not read enough "
                                    "pixel data!"
                                 << endl;
                        }
                        j = -1;
    ...
    bool BMP::CreateStandardColorTable(void)
    {
        ...
        if (BitDepth != 1 && BitDepth != 4 && BitDepth != 8)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: Attempted to create color table at a "
                        "bit"
                     << endl
                     << "                 depth that does not require a color "
                        "table."
                     << endl
    ...
    BMFH GetBMFH(const char* szFileNameIn)
    {
        ...
        if (!fp)
        {
            ...
            {
                cout
                    << "EasyBMP Error: Cannot initialize from file "
                    << szFileNameIn << "." << endl
    ...
    void DisplayBitmapInfo(const char* szFileNameIn)
    {
        ...
        cout << "File information for file " << szFileNameIn << ":" << endl
             << endl;
    ...
    bool Rescale(BMP& InputImage, char mode, int NewDimension)
    {
        ...
        if (CapMode != 'P' && CapMode != 'W' && CapMode != 'H' &&
            CapMode != 'F')
        {
            if (EasyBMPwarnings)
            {
        ...
        {
            ThetaJ =
                (double) (j * (OldHeight - 1.0)) / (double) (NewHeight - 1.0);
            J = (int) floor(ThetaJ);
            ThetaJ -= J;
            ...
            {
                ThetaI =
                    (double) (i * (OldWidth - 1.0)) / (double) (NewWidth - 1.0);
                I = (int) floor(ThetaI);
                ThetaI -= I;
