...
//#if 0       // DEBUG
//           std::cout << "prep_ports ::  row " << static_step << " column " << column << " in " << dst_size(static_step,column,0) << " out " << src_size(static_step,column,0) << std::endl;
        ...
        for (i=0;i<numvalues;i++) {
          ...
          if (step == 0)
          {
                ...
                for (unsigned long p=0; p < par->bilist[i].size() ; ++p)
                {
                    ...
                    j = par->bilist[i][p];
//                     std::cout << "prep_ports ::  j " << j << " i " << i << " p " << p << " par->bilist[i][p] " << par->bilist[i][p] << " par->bilist[i].size() " << par->bilist[i].size() <<std::endl;
                    if (i!=j)
                    {
                        vsrc_step.push_back(step);vsrc_column.push_back(i);vstep.push_back(dst);vcolumn.push_back(j);vport.push_back(counter);
                        counter++;   
