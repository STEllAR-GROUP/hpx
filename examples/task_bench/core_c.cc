...
interval_t wrap(std::pair<long, long> const& p)
{
    interval_t result;
...
task_graph_list_t wrap(std::vector<TaskGraph> const& g)
{
...
{
    TaskGraph t(graph);
    t.execute_point(timestep, point, reinterpret_cast<char*>(output_ptr),
        output_bytes,
        reinterpret_cast<char const**>(const_cast<int64_t const**>(input_ptr)),
