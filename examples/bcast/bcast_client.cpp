...
#include "bcast_hpx/server/partition.hpp"

/// This function initializes a vector of \a bcast::point clients,
...
{
    {
        ...
        std::size_t const sendbuf = vm["sendbuf"].as<std::size_t>();
        std::size_t const sendbuf2 = vm["sendbuf2"].as<std::size_t>();
        std::size_t const delay = vm["delay"].as<std::size_t>();

        // Get the component type for our point component.
        hpx::components::component_type block_type =
        hpx::components::get_component_type<bcast::server::partition>();
...
int main(int argc, char* argv[])
{
    ...
    desc_commandline.add_options()
        ...
        ("sendbuf", value<std::size_t>()->default_value(10),
            "the int and double array lengths for the broadcast")
        ("sendbuf2", value<std::size_t>()->default_value(10),
            "the double array length for the sendrecv")
        ("delay", value<std::size_t>()->default_value(0),
            "the delay iterations between the bcast and sendrecv");
