...
namespace policies
{
    ...
    // this
    // global bool variable. It will be set once by the runtime
    // configuration
    // startup code
    ...
    /// High priority threads are executed by the first N OS threads before
    /// any
    /// other work is executed. Low priority threads are executed by the
    /// last
    ...
    public:
        ...
        struct init_parameter
        {
            init_parameter()
              ...
              , numa_sensitive_(0)
              , description_("shared_priority_scheduler")
            {
            }
            ...
            init_parameter(std::size_t num_queues,
                ...
                char const* description = "shared_priority_scheduler")
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_pending_misses +=
                    high_priority_queue_.get_num_pending_misses(reset);
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_pending_accesses +=
                    high_priority_queue_.get_num_pending_accesses(reset);
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_stolen_threads +=
                    high_priority_queue_.get_num_stolen_from_pending(reset);
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_stolen_threads +=
                    high_priority_queue_.get_num_stolen_to_pending(reset);
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_stolen_threads +=
                    high_priority_queue_.get_num_stolen_from_staged(reset);
            ...
            std::size_t num_thread, bool reset)
        {
            ...
            if (num_thread == std::size_t(-1)) {
                num_stolen_threads +=
                    high_priority_queue_.get_num_stolen_to_staged(reset);
        ...
        bool cleanup_terminated(bool delete_all = false)
        {
            ...
            empty = high_priority_queue_.cleanup_terminated(delete_all) &&
                    empty;
...
//             }
            ...
            std::int64_t& idle_loop_count, threads::thread_data*& thrd)
        {
            ...
            // JB - no need to steal from another HP queue as HP queue is
            // shared anyway
            ...
            if (std::size_t(-1) != num_thread) {
                ...
                switch (priority) {
                    case thread_priority_default: {
                        count =
                            high_priority_queue_.get_thread_count(state);
                    ...
                    case thread_priority_low: {
                        if (queues_.size() - 1 == num_thread)
                            return low_priority_queue_.get_thread_count(
                                state);
                        break;
                    }
            ...
            thread_state_enum state = unknown) const
        {
            bool result = true;
            result =
                result && high_priority_queue_.enumerate_threads(f, state);
            ...
            std::size_t num_thread = std::size_t(-1)) const
        {
            ...
            if (std::size_t(-1) != num_thread) {
                ...
                wait_time =
                    high_priority_queue_->get_average_thread_wait_time();
                ++count;
            ...
            // Return the cumulative average thread wait time for all
            // queues.
            wait_time +=
                high_priority_queue_.get_average_thread_wait_time();
            ++count;
            ...
            std::size_t num_thread = std::size_t(-1)) const
        {
            ...
            if (std::size_t(-1) != num_thread) {
                ...
                wait_time =
                    high_priority_queue_->get_average_task_wait_time();
                ++count;
                ...
                wait_time +=
                    queues_[num_thread]->get_average_task_wait_time();
                return wait_time / (count + 1);
            }
        ...
        {
            //@TODO: JB Check this as we might create the same HP queue N
            //times
            ...
            for (std::size_t i = 0; i != num_threads; ++i) {
                std::size_t num_pu = get_resource_partitioner()
                                         .get_affinity_data()
                                         ->get_pu_num(i);
                ...
                core_masks[i] = get_resource_partitioner()
                                    .get_topology()
                                    .get_core_affinity_mask(
                                        num_pu, numa_sensitive_ != 0);
            }

            // iterate over the number of threads again to determine where
            // to
            // steal from
            ...
            // we allow the thread on the boundary of the NUMA domain to
            // steal
