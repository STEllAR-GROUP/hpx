...
static std::chrono::system_clock::time_point log_t_start = std::chrono::high_resolution_clock::now();

#define LOG_CUSTOM_WORKER(x) \
    dummy << "<CUSTOM> " << THREAD_ID << " time " << decimal(16) << nowt << ' '; \
    if (parent_pool) \
      dummy << "pool " \
            << std::setfill(' ') << std::setw(8) << parent_pool->get_pool_name() << " " \
            << x << std::endl; \
    else \
...
#define THREAD_DESC(thrd) \
    "\"" << thrd->get_description().get_description() << "\" " << hexpointer(thrd)

#define THREAD_DESC2(data, thrd) \
    "\"" << data.description.get_description() << "\" " << hexpointer(thrd?thrd->get():0)
...
namespace policies {
    ...
    public:
        ...
        std::int64_t get_num_pending_misses(std::size_t pool_queue_num, bool reset)
        {
            ...
            num_pending_misses += queues_[pool_queue_num]->
                get_num_pending_misses(reset);
...
//                get_num_pending_misses(reset);
        ...
        std::int64_t get_num_pending_accesses(std::size_t pool_queue_num, bool reset)
        {
...
//                    get_num_pending_accesses(reset);
            ...
            num_pending_accesses += queues_[pool_queue_num]->
                get_num_pending_accesses(reset);
...
//                get_num_pending_accesses(reset);
        ...
        std::int64_t get_num_stolen_from_pending(std::size_t pool_queue_num, bool reset)
        {
...
//                    get_num_stolen_from_pending(reset);
            ...
            num_stolen_threads += queues_[pool_queue_num]->
                get_num_stolen_from_pending(reset);
...
//                get_num_stolen_from_pending(reset);
        ...
        std::int64_t get_num_stolen_to_pending(std::size_t pool_queue_num, bool reset)
        {
            std::int64_t num_stolen_threads = 0;
            if (pool_queue_num == std::size_t(-1))
            {
...
//                get_num_stolen_to_pending(reset);
        ...
        std::int64_t get_num_stolen_from_staged(std::size_t pool_queue_num, bool reset)
        {
            std::int64_t num_stolen_threads = 0;
            if (pool_queue_num == std::size_t(-1))
            {
...
//                get_num_stolen_from_staged(reset);
        ...
        std::int64_t get_num_stolen_to_staged(std::size_t pool_queue_num, bool reset)
        {
            std::int64_t num_stolen_threads = 0;
            if (pool_queue_num == std::size_t(-1))
            {
...
//                get_num_stolen_to_staged(reset);
            ...
            std::size_t pool_queue_num)
        {
            ...
            if (pool_queue_num == std::size_t(-1)) {
                std::size_t t = threads::detail::thread_num_tss_.get_worker_thread_num();
                ...
                data.priority == thread_priority_boost)
            {
                ...
                LOG_CUSTOM_MSG("create_thread thread_priority_high "
                    ...
                    << "hp_queue_num " << hexnumber(pool_queue_num)
            ...
            if (data.priority == thread_priority_low) {
                ...
                LOG_CUSTOM_MSG("create_thread thread_priority_low "
                    << THREAD_DESC2(data, thrd)
                    << "pool_queue_num " << hexnumber(pool_queue_num)
                    << "scheduler " << hexpointer(data.scheduler_base));
                return;
            }
        ...
        virtual bool get_next_thread(std::size_t pool_queue_num, bool running,
            std::int64_t& idle_loop_count, threads::thread_data*& thrd)
        {
            ...
            if (!result) for (std::size_t i=0; i<high_priority_queues_.size(); ++i) {
                if (i!=tq) {
                    high_priority_queue = high_priority_queues_[i];
                    if (high_priority_queue->get_pending_queue_length()>1) {
                        result = high_priority_queue->get_next_thread(thrd, false, true);
                        if (result) break;
                    }
                }
            }
            ...
            thread_priority priority = thread_priority_normal)
        {
            ...
            {
                std::size_t hp_queue_num = curr_hp_queue_++ % high_priority_queues_.size();
            ...
            thread_priority priority = thread_priority_normal)
        {
                ...
                priority == thread_priority_boost)
            {
                LOG_CUSTOM_MSG("schedule_thread last thread_priority_high "
                    << THREAD_DESC(thrd));
                std::size_t hp_queue_num = curr_hp_queue_++ % high_priority_queues_.size();
                high_priority_queues_[hp_queue_num]->schedule_thread(thrd, true);
            }
            else if (priority == thread_priority_low)
            {
                ...
                LOG_CUSTOM_MSG("schedule_thread last thread_priority_normal "
                    << THREAD_DESC(thrd)
                    << "num_queue " << decnumber(num_queue));
        ...
        {
            LOG_CUSTOM_MSG("get_queue_length" << "pool_queue_num " << decnumber(pool_queue_num) );
            // Return queue length of one specific queue.
            std::int64_t count = 0;
            if (std::size_t(-1) != pool_queue_num) {
                HPX_ASSERT(pool_queue_num < queues_.size());

                auto high_priority_queue = high_priority_queues_[hp_queue_lookup_[pool_queue_num]];
        ...
        {
            LOG_CUSTOM_MSG("get_thread_count pool_queue_num " << hexnumber(pool_queue_num) );
            ...
            if (std::size_t(-1) != pool_queue_num) {
                ...
                switch (priority) {
                    ...
                    case thread_priority_high_recursive: {
                        return high_priority_queues_[hp_queue_lookup_[pool_queue_num]]
            ...
            thread_state_enum state = unknown) const
        {
            bool result = true;
            for (std::size_t i = 0; i != high_priority_queues_.size(); ++i) {
                result = result && high_priority_queues_[i]->enumerate_threads(f, state);
            }
        ...
        virtual bool wait_or_add_new(
            std::size_t pool_queue_num, bool running, std::int64_t& idle_loop_count)
        {
            std::size_t added  = 0;
            bool        result = true;
...
*/
            ...
            if (!initialized)
            {
                ...
                double s = high_priority_queues_.size();
                for (std::size_t i=0; i<num_queues; ++i) {
                    hp_queue_lookup_[i] = static_cast<int>(i*s/num_queues);
                }
            }
        }
