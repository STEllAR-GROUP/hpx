...
void print_count()
{
    ...
    cout << (boost::format("locality %1% (%2%, %3% shepherds) started receiving "
                             "%4% parcels and completed receiving %5% parcels")
              % a.get_prefix_id()
              % hostname
              % get_runtime().get_config().get_num_os_threads()
              % a.get_parcel_handler().get_parcelport().total_receives_started()
              % a.get_parcel_handler().get_parcelport().total_receives_completed())
           << endl;
}
