...
.. table:: Software Prerequisites for |hpx| on Linux systems.
    ...
    ====================== =================== =================== =====
    **Compilers**
    |gcc|                  4.9 or newer        4.9
    |icpc|                 2014 or newer       2014
    |clang|                3.8 or newer        3.8
    **Build System**
    |cmake|                3.9.0               3.3.2
    **Required Libraries**
    |boost_libraries|      1.62.0 or newer     1.58.0
    |hwloc|                2.0.1 or newer      1.2 (Xeon Phi: 1.6)
    ====================== =================== =================== =====
...
.. note::
   ...
   specify a ``cxxflags=-std=c++11`` command line argument to ``b2`` (``bjam``). Note
   however, that this is absolutely necessary when using gcc V5.2 and above.]
...
* Add CMake variable definitions (if any) by clicking the "Add Entry" button.
  ...
  folder of your |boost| and |hwloc| installations. It is recommended to set the
  variable ``CMAKE_INSTALL_PREFIX`` as well. This determines where the HPX
...
For more detailed information about using |cmake| please refer its documentation
and also the section :ref:`building_hpx`.
...
* Download the |hwloc| V1.11.0 (or latest version) from `here
  <http://www.open-mpi.org/software/hwloc/v1.11/downloads/hwloc-win64-build-1.11.0.zip>`__
  and unpack it.
* Download |boost| libraries V1.60 (or latest version) from `here
