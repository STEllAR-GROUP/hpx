...
* :hpx-issue:`5294` - execution::on should be based on execution::schedule
...
* :hpx-issue:`5232` - Annotation using hpx::util::annotated_function not working
...
* :hpx-pr:`5355` - Make the sequenced_executor processing_units_count member
  function const
* :hpx-pr:`5354` - Making sure default_stack_size is defined whenever declared
...
* :hpx-pr:`5348` - Rename tag_invoke and related facilities to tag_dispatch
...
* :hpx-pr:`5339` - Making sure hpx::util::hardware::timestamp() is always
  defined
* :hpx-pr:`5338` - Fixing timed_executor specializations of customization points
...
* :hpx-pr:`5326` - Adding resize() to serialize_buffer allowing to shrink its
  size
* :hpx-pr:`5324` - Add get member functions to async_rw_mutex proxy objects for
  explicitly getting the wrapped value
...
* :hpx-pr:`5308` - Seperate segmented algorithms for find, find_if, find_if_not
* :hpx-pr:`5307` - Seperate segmented algorithms for fill and generate
* :hpx-pr:`5304` - Fix compilation of sender CPOs with nvcc
* :hpx-pr:`5300` - Remove PRIVATE flag that was propagated into the LANGUAGES
...
* :hpx-pr:`5284` - Replacing util::function_nonser on std::function in hpx_init
...
* :hpx-pr:`5268` - Adding generic require, require_concept, and query properties
...
* :hpx-pr:`5224` - Separate segmented algorithms for none_of, any_of and all_of
...
* :hpx-pr:`5209` - Moving more files into modules (actions, components,
  init_runtime, etc.)
* :hpx-pr:`5208` - Add examples and explanation on when tag_fallback/priority
  are useful
...
* :hpx-pr:`5179` - Make sure --hpx:local command line option is respected with
  networking is off but distributed runtime is on
