...
The easiest way is to use the @c HPX_DECLARE_LOG* and @c HPX_DEFINE_LOG* macros:

@code
// in a header file
HPX_DECLARE_LOG_FILTER(g_log_filter, filter_type )
HPX_DECLARE_LOG(g_l, logger_type)

// in a source file
HPX_DEFINE_LOG_FILTER(g_log_filter, filter_type )
...
// define filter
HPX_DEFINE_LOG_FILTER(g_log_filter, finder::filter )
@endcode
...
// declare
HPX_DECLARE_LOG(g_log_err, finder::logger )

// define
HPX_DEFINE_LOG(g_log_err, finder::logger )
...
#include <hpx/util/logging/format_fwd.hpp>
// if you don't use formatters/destinations, you can include only <hpx/util/logging/logging.hpp>

// declare a filter, called g_log_filter
HPX_DECLARE_LOG_FILTER(g_log_filter, filter_type)

// declare a logger, called g_log
HPX_DECLARE_LOG(g_log, logger_type)
@endcode
...
HPX_DEFINE_LOG(g_log, logger_type)
@endcode
...
HPX_DEFINE_LOG_FILTER_WITH_ARGS(g_log_filter, filter_type, (level::debug, true) )

// define a logger, called g_log - assuming it needs an extra argument at construction
HPX_DEFINE_LOG_WITH_ARGS(g_log, logger_type, ("log.txt") )
@endcode
...
When using the @c HPX_DECLARE_LOG* and @c HPX_DEFINE_LOG* macros, this is what the lib takes care for you:
-# it declares and defines a logger/filter name, as a @b function (which internally contains a static variable)
-# cares about @ref HPX_LOG_COMPILE_FAST_ON "Fast Compile" (default) or @ref HPX_LOG_COMPILE_FAST_OFF "not"
...
The @c HPX_DECLARE_LOG* and @c HPX_DEFINE_LOG* macros take care of this automatically.
...
The @c HPX_DECLARE_LOG* and @c HPX_DEFINE_LOG* macros also automatically take care of fast compiling or not.
...
// in your code, only by #including hpx/util/logging/format_fwd.hpp, you can log messages
...
When using formatters/destinations, this means <tt>\#include <hpx/util/logging/format.hpp></tt>. Also, when logging a message,
...
When fast compile is off, HPX_DEFINE_LOG will generate code similar to this:

@code
logger_type * g_l() { static logger_type l; return &l; }
@endcode

When fast compile is on, HPX_DEFINE_LOG will generate code similar to this:
...
The @c HPX_DECLARE_LOG* and @c HPX_DEFINE_LOG* macros also automatically ensure that the logger/filter is instantiated before main().
...
Thus, you'll only need to #include <hpx/util/logging/format_fwd.hpp> throughout the application.

In case you're using formatters and destinations, you'll need to #include <hpx/util/logging/format.hpp> :
