...
-# <tt>HPX_LOG_USE_LOG_IF_FILTER(logger, filter_is_enabled)</tt>
-# <tt>HPX_LOG_USE_LOG_IF_LEVEL(logger, filter, level)</tt>
-# <tt>HPX_LOG_USE_SIMPLE_LOG_IF_FILTER(logger, filter_is_enabled)</tt>
...
- @c filter_is_enabled - how to determine if the filter is enabled.
  ...
  - As an example of filter that needs an extra argument, we have: \n
    <tt>HPX_LOG_USE_LOG_IF_LEVEL(l, holder, the_level) = HPX_LOG_USE_LOG_IF_FILTER(l, holder->is_enabled(the_level)) </tt>
...
HPX_DECLARE_LOG_FILTER(g_log_level, filter_type )
HPX_DECLARE_LOG(g_l, logger_type)
...
#define L_ HPX_LOG_USE_LOG_IF_FILTER(g_l(), g_log_filter()->is_enabled(__FILE__) )
@endcode
...
#define L_(mod) HPX_LOG_USE_LOG_IF_FILTER(g_l(), g_log_filter()->is_enabled(mod) )
@endcode
