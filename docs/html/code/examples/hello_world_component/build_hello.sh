...
# Compile the library
echo -o libhello_worldd.so hello_world_component.cpp `pkg-config --cflags --libs hpx_component` -DHPX_COMPONENT_NAME=hello_world
c++ -o libhello_worldd.so hello_world_component.cpp `pkg-config --cflags --libs hpx_component` -DHPX_COMPONENT_NAME=hello_world
...
# Compile the client
echo -o hello_world_client hello_world_client.cpp `pkg-config --cflags --libs hpx_application` -liostreamsd -lhello_worldd -L ~/my_hpx_libs
c++ -o hello_world_client hello_world_client.cpp `pkg-config --cflags --libs hpx_application` -liostreamsd -lhello_worldd -L ~/my_hpx_libs
