...
namespace hpx { namespace util
{
    namespace detail
    {
        ...
        ///////////////////////////////////////////////////////////////////////
        bool command_line_handling(std::vector<std::string>& ini_config, 
            util::manage_config& cfgmap, hpx::runtime_mode& mode, 
            hpx_main_type& f,boost::program_options::variables_map const& vm,
            std::size_t& num_threads, std::size_t& num_localities,
            std::string& queuing, std::size_t& node)
        {
        ...
        std::size_t& num_localities, std::string& queuing)
    {
...

        {
            ...
            variables_map prevm;
            cmd_result = util::parse_commandline(rtcfg, desc_cmdline, argc,
                argv, prevm, std::size_t(-1), util::allow_unregistered, mode);
            if (!cmd_result)
                return -1;
        ...
        cmd_result = util::parse_commandline(rtcfg, desc_cmdline,
            argc, argv, vm, std::size_t(-1), util::allow_unregistered, mode,
        ...
        // handle all straight --hpx:foo options
        std::size_t node = std::size_t(-1);
        cmd_result = detail::command_line_handling(ini_config, cfgmap, mode, 
            f, vm, num_threads, num_localities, queuing, node);
        if (!cmd_result)
            return -2;

        // how reparse the command line using the node number (if given)
        if (node != std::size_t(-1)) {
            cmd_result = util::parse_commandline(rtcfg, desc_cmdline,
                argc, argv, vm, node, util::allow_unregistered, mode,
                &help, &unregistered_options);
            if (!cmd_result)
                return -1;

            if (!unregistered_options.empty()) {
                unregistered_options_cmd_line.erase();
                typedef std::vector<std::string>::const_iterator iterator_type;
                iterator_type  end = unregistered_options.end();
                for (iterator_type  it = unregistered_options.begin(); it != end; ++it)
                    unregistered_options_cmd_line += " " + detail::enquote(*it);
            ...
            cmd_result = detail::command_line_handling(ini_config, cfgmap, 
                mode, f, vm, num_threads, num_localities, queuing, node);
            if (!cmd_result)
                return -2;
        }
