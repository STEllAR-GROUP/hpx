...
envfile=.jenkins/cscs/env-${configuration_name}.sh
...
#for domain in 128 256; do
  ...
  ${build_dir}/tools/perftests_ci/driver.py -v -l $logfile perftest run -o $result \
  ...
  ${build_dir}/tools/perftest_ci/driver.py -v -l $logfile perftest plot compare -i $reference $result -o reports-comparison-$configuration_name || { echo 'Plotting failed'; exit 1; }
#done
