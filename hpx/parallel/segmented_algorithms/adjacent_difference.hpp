...
#if !defined(HPX_PARALLEL_SEGMENTED_ALGORITHM_ADJACENT_DIFFERENCE_AUG_22_2017_1157AM)
            ...
            if (sit == send)
            {
                ...
                for (++sit, ++sdest; sit != send; ++sit, ++sdest)
                {
                    ...
                    if (beg != end)
                    {
                        ...
                        {
                            if(curr != end_dest)
                                *curr = hpx::util::invoke(op,
                                    *beginning, *std::prev(beginning));
                        }
                    }
                }
