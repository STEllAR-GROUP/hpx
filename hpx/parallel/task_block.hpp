...
namespace hpx { namespace parallel { HPX_INLINE_NAMESPACE(v2)
{
    ...
    namespace detail
    {
        ...
        template <typename F>
        struct task_block_result
        {
            typedef typename util::result_of<
                typename util::decay<F>::type(task_block&)
            >::type type;
        };

        template <typename F>
        typename task_block_result<F>::type
        define_task_block(F && f, boost::mpl::false_);

        template <typename F>
        void define_task_block(F && f, boost::mpl::true_);

        template <typename F>
        hpx::future<typename task_block_result<F>::type>
        define_async_task_block(F && f, boost::mpl::false_);

        template <typename F>
        hpx::future<void>
        define_async_task_block(F && f, boost::mpl::true_);
    }
    /// \endcond
    ...
    /// The class \a task_block defines an interface for forking and
    ...
    /// block library. Taking the address of a task_block object via
    ...
    /// define_task_block or define_task_block_restore_thread and
    /// "nearest enclosing" means the most
    ...
    /// block and a \a task_block passed to (or captured by) such code
    ...
    private:
        ...
        template <typename F>
        friend typename detail::task_block_result<F>::type
        detail::define_task_block(F &&, boost::mpl::false_);

        template <typename F>
        friend void detail::define_task_block(F &&, boost::mpl::true_);

        template <typename F>
        friend hpx::future<typename detail::task_block_result<F>::type>
        detail::define_async_task_block(F &&, boost::mpl::false_);

        template <typename F>
        friend hpx::future<void>
        detail::define_async_task_block(F &&, boost::mpl::true_);
        ...
        when(T && val, bool throw_on_error = false)
        {
            ...
            return hpx::lcos::local::dataflow(
                util::bind(util::one_shot(
                        &task_block::on_ready_val<result_type>),
    ...
    public:
        ...
        /// the same \a task_block or completion of the nearest enclosing
        /// task region (i.e., the \a define_task_block or
        /// \a define_task_block_restore_thread that
        /// created this \a task_block).
        ///
        /// Requires: F shall be MoveConstructible. The expression, (void)f(),
        ///           shall be well-formed.
        ///
        /// Precondition: this shall be the active \a task_block.
        ///
        ...
        void run(F && f)
        {
            // The proposal requires that the \a task_block should be
            // 'active' to be usable.
            ...
            hpx::future<void> result =
                hpx::async(hpx::launch::fork, boost::move(f));

            mutex_type::scoped_lock l(mtx_);
            tasks_.push_back(std::move(result));
        }

        /// Blocks until the tasks spawned using this \a task_block have
        /// finished.
        ///
        /// Precondition: this shall be the active \a task_block.
        ///
    ...
    namespace detail
    {
        ...
        define_task_block(F && f, boost::mpl::false_)
        {
            typedef typename task_block_result<F>::type result_type;
            ...
            try {
                ...
                task_block::mutex_type::scoped_lock l(trh.mtx_);
    ...
    ///
    /// \param f    The user defined function to invoke inside the \a task_block.
    ...
    ///
    template <typename F>
    typename detail::task_block_result<F>::type
    define_task_block(F && f)
    {
        typedef typename detail::task_block_result<F>::type result_type;
        typedef typename boost::is_void<result_type>::type is_void;

        return detail::define_task_block(std::forward<F>(f), is_void());
    }
    ...
    ///
    /// \tparam F   The type of the user defined function to invoke inside the
    ///             define task_block_restore_thread (deduced). \a F shall be
    ///             MoveConstructible.
    ///
    ...
    namespace detail
    {
        ...
        template <typename F>
        hpx::future<void> define_async_task_block(F && f, boost::mpl::true_)
        {
            task_block trh;
    ...
    ///
    template <typename F>
    hpx::future<typename detail::task_block_result<F>::type>
    define_async_task_block(F && f)
    {
        typedef typename detail::task_block_result<F>::type result_type;
        typedef typename boost::is_void<result_type>::type is_void;

        return detail::define_async_task_block(std::forward<F>(f), is_void());
    ...
    /// \param f    The user defined function to invoke inside the \a task_block.
    ///             Given an lvalue \a tr of type \a task_block, the
    ///             expression, f(tr), shall be well-formed.
    ///
    /// Postcondition: All tasks spawned from \a f have finished execution once
    ///                the returned future has become ready.
    ///                A call to \a task_block_restore_thread always returns on the
    ///                same thread as that on which it was called.
    ///
    /// \throws An \a exception_list, as specified in Exception Handling.
    ///
    /// \returns An instance of future<void> which will become ready once all
    ///          tasks spawned inside the \a task_block have finished
    ///          executing. Any exceptions thrown during execution of the
    ///          \a define_task_block or any of the spawned tasks are accessible
    ///          through the returned value as well.
    ///
    ...
    template <typename F>
    hpx::future<typename detail::task_block_result<F>::type>
    define_async_task_block_restore_thread(F && f)
    {
        // By design we always return on the same (HPX-) thread as we started
        // executing define_task_block_restore_thread.
        return define_async_task_block(std::forward<F>(f));
    }
}}}
