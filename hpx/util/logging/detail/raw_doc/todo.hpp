...
- @c normal         make it so that I use HPX_LOG_USE_WCHAR_T as little as possible
...
- @c low            when leaking/not leaking loggers/using after_destroyed - have a templated class that allows this
                    ...
                    for macros to be visible in client code (for instance, if client wants HPX_LOG_BEFORE_INIT_USE_CACHE_FILTER)
...
- @c normal         explain HPX_LOG_USE_SIMPLE_LOG_IF_FILTER (defining_logger_macros)
