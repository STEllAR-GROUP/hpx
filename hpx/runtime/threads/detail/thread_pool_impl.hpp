...
namespace hpx { namespace threads { namespace detail
{
    ...
    public:
            ...
            thread_state_enum initial_state, error_code& ec)
        {
            ...
            detail::create_work(
                sched_.get(), data, initial_state, ec);    //-V601
        }
            ...
            std::size_t pool_threads)
        {
            ...
            try {
                ...
                {
                    std::size_t global_thread_num = thread_offset_ + thread_num;
                    ...
                    threads_.push_back(compat::thread(&thread_pool::thread_func,
                        this, thread_num, std::ref(topology_),
                        std::ref(startup)));
            ...
            if (!threads_.empty())
            {
                ...
                if (blocking)
                {
                    for (std::size_t i = 0; i != threads_.size(); ++i)
                    {
                        ...
                        // unlock the lock while joining
                        util::unlock_guard<std::unique_lock<compat::mutex>> ul(
                            l);
                        threads_[i].join();
                    }
                    threads_.clear();
        ...
        {
            std::size_t global_thread_num = thread_num + thread_offset_;
            ...
            init_tss_helper<Scheduler> tss_helper(*this, thread_num, thread_offset_);
...

            {
                ...
                try
                {
                    try
                    {
                        ...
                        HPX_ASSERT(!sched_->Scheduler::get_thread_count(unknown,
    ...
    private:
        ...
        std::size_t thread_offset_;    // is equal to the accumulated number of
