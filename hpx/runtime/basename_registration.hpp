...
namespace hpx
{
    ...
    HPX_API_EXPORT std::vector<hpx::future<hpx::id_type> >
        find_all_from_basename(char const* base_name, std::size_t num_ids);
    ...
    ///
    HPX_API_EXPORT std::vector<hpx::future<hpx::id_type> >
        find_from_basename(char const* base_name,
    ...
    ///
    template <typename Client>
    Client find_from_basename(char const* base_name,
        std::size_t sequence_nr = ~0U)
    {
        return components::make_client<Client>(
            find_from_basename(base_name, sequence_nr));
    }
    ...
    hpx::future<bool> register_with_basename(char const* base_name,
