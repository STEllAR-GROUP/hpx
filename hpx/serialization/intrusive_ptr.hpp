...
namespace hpx { namespace serialization {
    ...
    namespace detail
    {
        ...
        template <class T>
        class intrusive_ptr_input_dispatcher
        {
          struct intrusive_polymorphic
          {
            static void serialize(input_archive& ar,
                boost::intrusive_ptr<T>& ptr, boost::uint64_t pos)
            {
              std::string name;
              ar >> name;
        ...
        template <class T>
        class intrusive_ptr_output_dispatcher
        {
          struct intrusive_polymorphic
          {
    ...
    void serialize(input_archive & ar, boost::intrusive_ptr<T> & ptr, unsigned)
    {
        ...
        if(valid)
        {
            ...
            if(pos == boost::uint64_t(-1))
            {
                pos = 0;
                ar >> pos;
                detail::intrusive_ptr_input_dispatcher<T>::type::serialize(ar, ptr, pos);
            }
            else
            {
    ...
    void serialize(output_archive & ar, boost::intrusive_ptr<T> ptr, unsigned)
    {
        ...
        if(valid)
        {
            ...
            {
                ar << cur_pos;
                detail::intrusive_ptr_output_dispatcher<T>::type::serialize(ar, ptr);
            }
        }
    }
}}
