...
namespace hpx { namespace performance_counters { namespace papi {
    namespace util {
        ...
        std::map<std::string, int> const papi_domain_map = {
            {"user", PAPI_DOM_USER}, {"kernel", PAPI_DOM_KERNEL},
            {"other", PAPI_DOM_OTHER}, {"super", PAPI_DOM_SUPERVISOR},
            {"all", PAPI_DOM_ALL}, {"min", PAPI_DOM_MIN},
            {"max", PAPI_DOM_MAX}};
        ...
        options_description get_options_description()
        {
            ...
            papi_opts.add_options()("hpx:papi-domain",
                value<std::string>()->default_value("user"),
                ...
                "  max    - largest available context.")("hpx:papi-multiplex",
                value<long>()->implicit_value(0),
                "enable low level counter multiplexing.")("hpx:papi-event-info",
                value<std::string>()->implicit_value("preset"),
                "display detailed information about events available on local "
                "host;"
        ...
        {
            std::string s;
            if (t & PAPI_PRESET_BIT_MSC)
                s += ",OTHER";
            if (t & PAPI_PRESET_BIT_INS)
                s += ",INS";
            if (t & PAPI_PRESET_BIT_IDL)
                s += ",IDLE";
            if (t & PAPI_PRESET_BIT_BR)
                s += ",BRANCH";
            if (t & PAPI_PRESET_BIT_CND)
                s += ",COND";
            if (t & PAPI_PRESET_BIT_MEM)
                s += ",MEM";
            if (t & PAPI_PRESET_ENUM_CACH)
                s += ",CACHE";
            if (t & PAPI_PRESET_BIT_L1)
                s += ",L1";
            if (t & PAPI_PRESET_BIT_L2)
                s += ",L2";
            if (t & PAPI_PRESET_BIT_L3)
                s += ",L3";
            if (t & PAPI_PRESET_BIT_TLB)
                s += ",TLB";
            if (t & PAPI_PRESET_BIT_FP)
                s += ",FP";
            if (!s.empty())
                return s.substr(1);
            s = "-";
        ...
        void list_presets()
        {
                ...
                *it != nullptr; ++it)
            {
                hpx::util::format_to(std::cout,
                    ...
                    "Derived from : {}\n"
                    "Description  : {}\n",
                    (*it)->symbol, decode_preset_type((*it)->event_type),
                    (*it)->event_code, dependencies(**it), (*it)->long_descr);
        ...
        void print_native_info(PAPI_event_info_t const& info)
        {
            hpx::util::format_to(std::cout,
                ...
                "Derived from : {}\n"
                "Description  : {}\n",
                info.symbol, info.event_code, registers(info), info.long_descr);
        ...
        void list_native()
        {
            ...
            for (int ci = 0; ci < PAPI_num_components(); ++ci)
            {
                ...
                for (auto it =
                         hpx::util::generator_iterator<native_info_gen>(&gen);
                    *it != nullptr; ++it)
                {
        ...
        void list_events(std::string const& scope)
        {
            ...
            {
                hdr += hpx::util::format(
                    "{} (locality {}):", host, hpx::get_locality_id());
            }
            else
            {
                hdr +=
                    hpx::util::format("locality {}:", hpx::get_locality_id());
            }
            std::cout << hdr << std::endl
                      << std::string(hdr.length(), '=') << std::endl;
            if (scope == "preset" || scope == "all")
                list_presets();
        ...
        std::uint32_t get_counter_thread(
            counter_path_elements const& cpe, std::string& label)
        {
            ...
            {
                label = hpx::util::format(
                    "{}#{}", cpe.instancename_, cpe.instanceindex_);
            }
