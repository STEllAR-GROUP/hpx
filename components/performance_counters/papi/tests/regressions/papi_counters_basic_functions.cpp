...
int hpx_main(hpx::program_options::variables_map&)
{
    ...
    bool pass = status_is_valid(value1.status_) &&
        status_is_valid(value2.status_) && status_is_valid(value3.status_) &&
    ...
    if (pass)
    {
        ...
        std::cout << n << " counted fp instructions, result: " << cnt1
                  << std::endl
                  << n << " uncounted fp instructions, result: " << cnt2 - cnt1
                  << std::endl
                  << 2 * n << " fp instructions, count after reset: " << cnt3
                  << std::endl
                  << n
                  << " fp instructions, count after explicit reset: " << cnt4
                  << std::endl;

        pass = pass && close_enough(cnt1, n, 1.0) && (cnt2 >= cnt1) &&
            close_enough(cnt1, cnt2, 1.0) &&
