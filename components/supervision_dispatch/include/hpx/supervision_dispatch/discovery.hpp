...
/// \page hpx::supervision::discover_peers
/// \page hpx::supervision::fan_out_join
/// \page hpx::supervision::discover_and_join
...
namespace hpx::supervision {
    ...
    // Locality discovered_peer::join_epoch value meaning "never joined" (see
    ...
    // A peer whose supervision_dispatch registry name (see
    // registry::register_name()) was successfully resolved by discover_peers().
    struct discovered_peer
    {
        ...
        // local_registry.join(peer_locality) call that fan_out_join()/
        // discover_and_join() already perform internally. Left at its default
        // (unjoined_epoch) for peers returned by discover_peers() alone, which
        // never calls join(). Together with locality - which is already the
        // same value as joined_peer::target - this carries everything
    ...
    /// concurrently resolves the pinned registry name (see register_name()) of
    /// every remote locality (hpx::find_remote_localities(), which already
    /// excludes this locality), bounded by a single wait for \a timeout across
    /// all candidates.
    ///
    /// Because constructing a registry client from a symbolic name (see
    /// client_base's symbolic-name constructor) waits/resolves once the symbol
    /// is bound rather than failing fast on one that is not (yet) registered,
    /// localities that never called register_name() (e.g. because they have not
    /// run init_supervision() yet) would otherwise hang this call indefinitely.
    /// Instead, once \a timeout elapses, only the candidates whose registry
    /// client was resolved by then (checked via the non-blocking is_ready())
    /// are returned; every other candidate is simply excluded as not currently
    /// participating in supervision - discover_peers() itself never fails or
    /// throws on their account.
    ///
    /// \param timeout  The maximum duration to wait, across all candidates
    ///                 combined, for their registry names to resolve.
    ///
    /// \return         The peers whose registry client was resolved within \a
    ///                 timeout, as discovered_peer values carrying the
    ///                 hpx::id_type locality it is pinned to.
    ...
    ///                 throwing.
    ///
    /// \return The discovered_peer values (locality + join_epoch) for peers
    ///         whose join() calls settled successfully within \a timeout, in
    ///         the same relative order as \a peers (with unsuccessful/
    ///         timed-out peers omitted, rather than left as gaps). Each
    ///         returned discovered_peer::join_epoch is populated from the
    ///         corresponding join() call, so callers can dispatch against
    ///         `{peer.locality, peer.join_epoch}` directly, without needing a
    ///         separate join() call to obtain it.
    ...
    /// to resolve the pinned registry name of every remote locality
    /// (hpx::find_remote_localities() already excludes this locality, so no
    /// separate self-exclusion is needed here), then fans the resulting peer
    /// list out to \a local_registry via a single fan_out_join() pass.
    ///
    ...
    /// accident - is safe and never creates more than one registry entry per
    /// peer locality; repeated invocations simply return the (stable) join
    /// epochs already owned for peers that were previously joined.
    ///
    ...
    ///                       candidates combined, for their registry name to
    ///                       resolve (see discover_peers()).
    ///
    /// \return The discovered_peer value that \a local_registry created (or
