...
namespace hpx { namespace util {
    ...
    HPX_EXPORT apex_task_wrapper apex_new_task(std::size_t address,
        std::uint32_t parent_task_locality,
        threads::thread_id_type const& parent_task);

    HPX_EXPORT apex_task_wrapper apex_new_task(char const* description,
        std::uint32_t parent_task_locality,
        threads::thread_id_type const& parent_task);

    inline apex_task_wrapper apex_update_task(
        apex_task_wrapper wrapper, std::size_t address)
    {
        if (wrapper == nullptr)
        {
    ...
    inline apex_task_wrapper apex_update_task(
        apex_task_wrapper wrapper, char const* description)
    {
        if (wrapper == nullptr)
        {
            threads::thread_id_type parent_task(nullptr);
...
#else
    inline apex_task_wrapper apex_new_task(std::size_t address,
        std::uint32_t parent_task_locality,
        threads::thread_id_type const& parent_task)
    {
        return nullptr;
    ...
    inline apex_task_wrapper apex_new_task(char const* description,
        std::uint32_t parent_task_locality,
        threads::thread_id_type const& parent_task)
    {
        return nullptr;
