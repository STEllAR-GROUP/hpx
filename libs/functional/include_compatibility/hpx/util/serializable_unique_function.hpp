//  Copyright (c) 2019 Ste||ar Group
//
//  SPDX-License-Identifier: BSL-1.0
//  Distributed under the Boost Software License, Version 1.0. (See accompanying
//  file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)

#pragma once

#include <hpx/config.hpp>
#include <hpx/functional/config/defines.hpp>
#include <hpx/functional/serialization/serializable_unique_function.hpp>

#if defined(HPX_FUNCTIONAL_HAVE_DEPRECATION_WARNINGS)
#if defined(HPX_MSVC)
#pragma message(                                                               \
    "The header hpx/util/serializable_unique_function.hpp is deprecated, \
    please include hpx/functional/serialization/serializable_unique_function.hpp instead")
#else
#warning                                                                       \
    "The header hpx/util/serializable_unique_function.hpp is deprecated, \
    please include hpx/functional/serialization/serializable_unique_function.hpp instead"
#endif
#endif
