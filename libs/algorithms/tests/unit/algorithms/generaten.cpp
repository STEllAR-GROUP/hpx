...
void test_generate_n(ExPolicy policy, IteratorTag)
{
    ...
    hpx::parallel::generate_n(policy, iterator(std::begin(c)), c.size(), gen);
...
void test_generate_n_async(ExPolicy p, IteratorTag)
{
    ...
    hpx::future<iterator> f =
        hpx::parallel::generate_n(p, iterator(std::begin(c)), c.size(), gen);
    f.wait();
...
void test_generate_n_exception_async(ExPolicy p, IteratorTag)
{
    ...
    try
    {
        hpx::future<decorated_iterator> f = hpx::parallel::generate_n(p,
            decorated_iterator(
                std::begin(c), []() { throw std::runtime_error("test"); }),
            c.size(), gen);
