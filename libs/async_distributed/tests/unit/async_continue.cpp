...
int hpx_main()
{
    ...
    if (!localities.empty())
    {
        ...
        hpx::future<int> f2 =
            hpx::async_continue(inc_f, make_continuation(), localities[0], f);
    ...
    {
        hpx::future<int> f = hpx::async_continue(
            inc, make_continuation(mult, localities[0]), localities[0], 42);
        HPX_TEST_EQ(f.get(), 86);
        ...
        f = hpx::async_continue(
            inc, make_continuation(mult), localities[0], 42);
        HPX_TEST_EQ(f.get(), 86);
