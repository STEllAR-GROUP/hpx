...
template <class T>
void test_conversion_from_integral_to_char(char zero)
{
...
template <class T>
void test_conversion_from_char_to_integral(char zero)
{
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 0)) == static_cast<T>(0));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 1)) == static_cast<T>(1));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 2)) == static_cast<T>(2));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 3)) == static_cast<T>(3));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 4)) == static_cast<T>(4));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 5)) == static_cast<T>(5));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 6)) == static_cast<T>(6));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 7)) == static_cast<T>(7));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 8)) == static_cast<T>(8));
    HPX_TEST(lexical_cast<T>(static_cast<char>(zero + 9)) == static_cast<T>(9));
...
template <class T>
void test_conversion_from_integral_to_string(char)
{
    ...
    t = (limits::min)();
    HPX_TEST(lexical_cast<string_type>(t) == to_str<char>(t));
...
template <class T>
void test_conversion_from_string_to_integral(char)
{
    typedef std::numeric_limits<T> limits;
    typedef std::string string_type;

    string_type s;
    string_type const zero = to_str<char>(0);
    string_type const nine = to_str<char>(9);
    T const min_val = (limits::min)();
    T const max_val = (limits::max)();

    s = to_str<char>(min_val);
    ...
    HPX_TEST_EQ(lexical_cast<T>(s), max_val);
    {
        ...
        {
            HPX_TEST_THROW(
                lexical_cast<T>(to_str<char>(i) + s), bad_lexical_cast);
        }
        ...
        if (!(std::is_same<T, promoted>::value))
        {
            ...
            {
                HPX_TEST_THROW(
                    lexical_cast<T>(to_str<char>(prom + i)), bad_lexical_cast);
                HPX_TEST_THROW(
                    lexical_cast<T>(to_str<char>(i) + s), bad_lexical_cast);
            }
        }
    ...
    if (limits::digits <= 16 && lcast_test_small_integral_types_completely)
        ...
        for (T t = 1 + min_val; t != max_val; ++t)
            HPX_TEST(lexical_cast<T>(to_str<char>(t)) == t);
    else
    {
        T const half_max_val = max_val / 2;
        T const cnt = lcast_integral_test_counter;    // to suppress warnings
