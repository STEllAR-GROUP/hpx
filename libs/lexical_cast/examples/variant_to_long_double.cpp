...
/*`
    In this example we'll make a `to_long_double` method that converts value
    of the Boost.Variant to `long double`.
*/
...
struct to_long_double_functor : boost::static_visitor<long double>
{
    ...
    long double operator()(const T& v) const
    {
        ...
        // situations that usually occur in generic programming, like
        // std::string to std::string or arithmetic type to arithmetic type
        // conversion.
        return hpx::util::lexical_cast<long double>(v);
    }
};

// Throws `hpx::util::bad_lexical_cast` if value of the variant is not
