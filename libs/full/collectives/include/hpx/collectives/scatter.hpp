...
namespace hpx::collectives {
        ...
        generation_arg const generation = generation_arg())
    {
        ...
        for (std::size_t i = 1; i != communicators.size() - 1; ++i)
        {
            data = scatter_to(hpx::launch::sync, communicators.get(i),
                detail::scatter_data(HPX_MOVE(data), communicators.arity),
        ...
        generation_arg const generation = generation_arg())
    {
        ...
        arity_arg arity = communicators.arity;
        std::vector<T> data =
    ...
    ////////////////////////////////////////////////////////////////////////////
    template <typename T>
    T scatter_from(hpx::launch::sync_policy,
        hierarchical_communicator const& communicators,
        this_site_arg const this_site = this_site_arg(),
        generation_arg const generation = generation_arg())
    {
        return scatter_from<T>(communicators, this_site, generation).get();
    }

    template <typename T>
    T scatter_to(hpx::launch::sync_policy,
        hierarchical_communicator const& communicators,
        std::vector<T>&& local_result,
        this_site_arg const this_site = this_site_arg(),
        generation_arg const generation = generation_arg())
