    ...
    for (std::uint32_t i = 0; i != ITERATIONS; ++i)
    {
        if (this_locality == 0)
        {
            ...
            hpx::future<std::uint32_t> result =
                scatter_to(scatter_clients, std::move(data));
        ...
        {
            hpx::future<std::uint32_t> result =
                scatter_from<std::uint32_t>(scatter_clients);
...
void test_multiple_use_with_generation(int arity = 2)
{
    ...
    for (std::uint32_t i = 0; i != ITERATIONS; ++i)
    {
        if (this_locality == 0)
        {
            ...
            hpx::future<std::uint32_t> result = scatter_to(scatter_clients,
                std::move(data), this_site_arg(), generation_arg(i + 1));
        ...
        else
        {
            hpx::future<std::uint32_t> result = scatter_from<std::uint32_t>(
                scatter_clients, this_site_arg(), generation_arg(i + 1));
