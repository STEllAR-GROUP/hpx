...
void test_one_shot_use()
{
    ...
    for (int i = 0; i != ITERATIONS; ++i)
    {
        std::uint32_t overall_result =
            all_reduce(hpx::launch::sync, all_reduce_direct_basename, here + i,
...
void test_multiple_use()
{
    ...
    for (int i = 0; i != ITERATIONS; ++i)
    {
        std::uint32_t overall_result = all_reduce(hpx::launch::sync,
...
void test_multiple_use_with_generation()
{
    ...
    {
        std::uint32_t overall_result =
            all_reduce(hpx::launch::sync, all_reduce_direct_client, here + i,
    ...
    for (std::uint32_t site = 0; site != num_sites; ++site)
    {
        sites.push_back(hpx::async([=]() {
            ...
            for (std::uint32_t i = 0; i != 10 * ITERATIONS; ++i)
            {
                ...
                std::uint32_t result = all_reduce(hpx::launch::sync,
                    all_reduce_direct_client, value, std::plus<>{},
