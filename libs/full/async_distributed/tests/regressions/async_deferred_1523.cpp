...
{
    {
        ...
        int result =
            hpx::async<int_action>(hpx::launch::deferred, hpx::find_here())
                .get();
        ...
        {
            hpx::id_type id =
                hpx::async<get_locality_action>(hpx::launch::deferred, loc)
                    .get();
            HPX_TEST_EQ(loc, id);
        }
    }
    ...
    {
        auto nt = hpx::actions::lambda_to_action([]() { nt_executed = true; });

        auto it = hpx::actions::lambda_to_action([]() -> int {
            it_executed = true;
            return 42;
        });

        auto gl = hpx::actions::lambda_to_action(
            []() -> hpx::id_type { return hpx::find_here(); });

        hpx::async(hpx::launch::deferred, std::move(nt), hpx::find_here())
            .get();
        HPX_TEST(nt_executed);

        int result =
            hpx::async(hpx::launch::deferred, std::move(it), hpx::find_here())
                .get();
        ...
        for (hpx::id_type const& loc : hpx::find_all_localities())
        {
            hpx::id_type id = hpx::async(hpx::launch::deferred, gl, loc).get();
            HPX_TEST_EQ(loc, id);
        }
    }
#endif
