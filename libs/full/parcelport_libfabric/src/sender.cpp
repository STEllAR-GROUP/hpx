...
namespace hpx::parcelset::policies::libfabric {
    ...
    void sender::async_write_impl()
    {
        ...
        for (auto& c : buffer_.chunks_)
        {
            ...
            if (c.type_ == serialization::chunk_type::chunk_type_pointer)
            {
                ...
                LOG_DEBUG_MSG("Created zero-copy rdma Get region "
                    << decnumber(index) << *zero_copy_region << "for rkey "
                    << hexpointer(c.rkey_));

                LOG_TRACE_MSG(CRC32_MEM(zero_copy_region->get_address(),
        ...
        LOG_DEBUG_MSG("sender "
            << hexpointer(this) << ", buffsize "
            << hexuint32(header_->message_size()) << ", header_length "
            << decnumber(header_->header_length()) << ", chunks zerocopy( "
            << decnumber(buffer_.num_chunks_.first) << ") "
        ...
        LOG_DEBUG_MSG("Found region allocated during encode_parcel : address "
            << hexpointer(buffer_.data_.m_array_) << " region "
            << *message_region_);
        ...
        {
            LOG_DEBUG_MSG(
                "Sender " << hexpointer(this) << "Chunk info is piggybacked");
        }
        else
        {
            ...
            LOG_DEBUG_MSG("Set up header-chunk rma data with "
                << "size " << decnumber(cb.size_) << "rkey "
                << hexpointer(cb.rkey_) << "addr "
                << hexpointer(cb.data_.cpos_));
        }

        if (header_->message_piggy_back())
        {
            LOG_DEBUG_MSG(
                "Sender " << hexpointer(this) << "Main message is piggybacked");
        ...
        {
            header_->set_message_rdma_info(message_region_->get_remote_key(),
                message_region_->get_address());

            LOG_DEBUG_MSG("Sender "
                << hexpointer(this) << "message region NOT piggybacked "
                << hexnumber(buffer_.data_.size()) << *message_region_);
            ...
            hpx::util::yield_while(
                [this]() {
                    int ret =
                        fi_send(this->endpoint_, this->region_list_[0].iov_base,
                            this->region_list_[0].iov_len, this->desc_[0],
                            this->dst_addr_, this);
    ...
    {
        LOG_DEBUG_MSG("Sender "
            << hexpointer(this) << "handle send_completion "
    ...
    {
        LOG_DEBUG_MSG("Sender "
            << hexpointer(this) << "handle handle_message_completion_ack ( "
            << "RMA regions " << decnumber(rma_regions_.size())
            << "completion count " << decnumber(completion_count_));
        ++acks_received_;
        cleanup();
    ...
    void sender::cleanup()
    {
        LOG_DEBUG_MSG("Sender " << hexpointer(this)
                                << "decrementing completion_count from "
                                << decnumber(completion_count_));
    ...
    // --------------------------------------------------------------------
    void sender::handle_error(struct fi_cq_err_entry)
    {
