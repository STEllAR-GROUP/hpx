...
#include <hpx/parcelport_libfabric/rma_receiver.hpp>
...
namespace hpx { namespace parcelset { namespace policies { namespace libfabric {
    ...
    public:
        ...
        // lock types for maps
        typedef hpx::concurrent::unordered_map<uint32_t,
            promise_tuple_type>::map_read_lock_type map_read_lock_type;
        typedef hpx::concurrent::unordered_map<uint32_t,
            promise_tuple_type>::map_write_lock_type map_write_lock_type;
        ...
        libfabric_controller(std::string const& provider,
            std::string const& domain, std::string const& endpoint,
            int /*port*/ = 7910)
        ...
        void boot_PMI()
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                LOG_DEBUG_MSG("Calling insert_address for "
                    << decnumber(i) << "on rank " << decnumber(rank));
                insert_address(new_locality);
                LOG_DEBUG_MSG(
                    "rank " << decnumber(i) << "added to address vector");
                if (i == 0)
                {
                    agas_ = new_locality;
                }
        ...
        ~libfabric_controller()
        {
            ...
            LOG_DEBUG_MSG("Received messages "
                << decnumber(messages_handled) << "Received acks "
                << decnumber(acks_received) << "Sent acks "
                << decnumber(sent_ack) << "Total reads " << decnumber(rma_reads)
                << "Total deletes " << decnumber(recv_deletes)
                << "deletes error "
                << decnumber(messages_handled - recv_deletes));
            ...
            std::string const& endpoint_type)
        {
            ...
            fabric_hints_->caps = FI_MSG | FI_RMA | FI_SOURCE | FI_WRITE |
            ...
            LOG_DEBUG_MSG(
                "fabric provider " << fabric_hints_->fabric_attr->prov_name);
            if (domain.size() > 0)
            {
                fabric_hints_->domain_attr->name = strdup(domain.c_str());
                LOG_DEBUG_MSG(
                    "fabric domain " << fabric_hints_->domain_attr->name);
            }
            ...
            int ret = fi_getinfo(FI_VERSION(1, 4), nullptr, nullptr, flags,
                fabric_hints_, &fabric_info_);
            if (ret)
            {
                throw fabric_error(ret, "Failed to get fabric info");
            }
            LOG_DEBUG_MSG(
                "Fabric info " << fi_tostr(fabric_info_, FI_TYPE_INFO));
...
#else
            bind_passive_endpoint_to_queues(ep_passive_);
        ...
        void create_event_queue()
        {
            ...
            if (ret)
                throw fabric_error(ret, "fi_eq_open");
        ...
        locality create_local_endpoint()
        {
            ...
            LOG_DEBUG_MSG(
                "Fetching local address using size " << decnumber(addrlen));
            ...
            struct fi_info* info, struct fid_ep** new_endpoint)
        {
            ...
            LOG_DEBUG_MSG("Creating active endpoint");
            LOG_DEBUG_MSG(
                "Got info mode " << (info->mode & FI_NOTIFY_FLAGS_ONLY));
            int ret = fi_endpoint(fabric_domain_, info, new_endpoint, nullptr);
            if (ret)
                throw fabric_error(ret, "fi_endpoint");
            ...
            if (info->ep_attr->type == FI_EP_MSG)
            {
                if (event_queue_)
                {
                    ...
                    if (ret)
                        throw fabric_error(ret, "bind event_queue_");
                }
            }
        }
        ...
        void bind_endpoint_to_queues(struct fid_ep* endpoint)
        {
            ...
            if (txcq_)
            {
                ...
                if (ret)
                    throw fabric_error(ret, "bind txcq");
            }
            ...
            if (ep_shared_rx_cxt_)
            {
                ...
                if (ret)
                    throw fabric_error(ret, "ep_shared_rx_cxt_");
            }
        ...
        void bind_passive_endpoint_to_queues(struct fid_pep* endpoint)
        {
            ...
            if (av_)
            {
                LOG_DEBUG_MSG("Binding passive endpoint to AV");
                ret = fi_pep_bind(endpoint, &av_->fid, 0);
                if (ret)
                    throw fabric_error(ret, "bind event_queue_");
            }
            ...
            if (txcq_)
            {
                LOG_DEBUG_MSG("Binding passive endpoint to TX CQ");
                ret = fi_pep_bind(endpoint, &txcq_->fid, FI_TRANSMIT);
                if (ret)
                    throw fabric_error(ret, "bind txcq");
            }
            ...
            if (rxcq_)
            {
                LOG_DEBUG_MSG("Binding passive endpoint to RX CQ");
                ret = fi_pep_bind(endpoint, &rxcq_->fid, FI_RECV);
                if (ret)
                    throw fabric_error(ret, "rxcq");
            }
            ...
            {
                LOG_DEBUG_MSG(
                    "Binding passive endpoint to shared receive context");
                ret = fi_pep_bind(endpoint, &ep_shared_rx_cxt_->fid, 0);
                if (ret)
                    throw fabric_error(ret, "ep_shared_rx_cxt_");
            }
        ...
        void initialize_localities()
        {
            ...
            LOG_DEBUG_MSG(
                "Parcelport initialize_localities with " << N << " localities");
            ...
            {
                hpx::naming::gid_type l =
                    hpx::naming::get_gid_from_locality_id(i);
                LOG_DEBUG_MSG("Resolving locality" << l);
                // each locality may be reachable by mutiplte parcelports
                const parcelset::endpoints_type& res =
                    agas::resolve_locality(l);
                // get the fabric related data
                auto it = res.find("libfabric");
                LOG_DEBUG_MSG("locality resolution " << it->first << " => "
                                                     << it->second);
        ...
        {
            uint64_t key =
                (uint64_t) dest_fabric.ip_address() << 32 | dest_fabric.port();
        ...
        int poll_send_queue()
        {
            LOG_TIMED_INIT(poll);
            LOG_TIMED_BLOCK(
                poll, DEVEL, 5.0, { LOG_DEBUG_MSG("poll_send_queue"); });

            fi_cq_msg_entry entry;
            int ret = 0;
            {
                std::unique_lock<mutex_type> l(
                    polling_mutex_, std::try_to_lock);
                if (l)
            ...
            if (ret > 0)
            {
                LOG_DEBUG_MSG("Completion txcq wr_id "
                    ...
                    << "context " << hexpointer(entry.op_context) << "length "
                    << hexuint32(entry.len));
                if (entry.flags & FI_RMA)
                {
                    ...
                    rma_receiver* rcv =
                        reinterpret_cast<rma_receiver*>(entry.op_context);
                ...
                else if (entry.flags == (FI_MSG | FI_SEND))
                {
                    ...
                    sender* handler =
                        reinterpret_cast<sender*>(entry.op_context);
                    handler->handle_send_completion();
                }
                else
                {
                    LOG_DEBUG_MSG(
                        "$$$$$ Received an unknown txcq completion ***** "
                        << decnumber(entry.flags));
                    std::terminate();
                }
                return 1;
            ...
            else if (ret == -FI_EAVAIL)
            {
                ...
                if (e.err == err_sz)
                {
                    LOG_ERROR_MSG("txcq_ Error with len "
                        << hexlength(e.len) << "context "
                        << hexpointer(e.op_context));
                }
                // flags might not be set correctly
                if (e.flags == (FI_MSG | FI_SEND))
                {
                    LOG_ERROR_MSG("txcq Error for FI_SEND with len "
                        << hexlength(e.len) << "context "
                        << hexpointer(e.op_context));
                }
                if (e.flags & FI_RMA)
                {
                    LOG_ERROR_MSG("txcq Error for FI_RMA with len "
                        << hexlength(e.len) << "context "
                        << hexpointer(e.op_context));
                }
        ...
        int poll_recv_queue()
        {
            LOG_TIMED_INIT(poll);
            LOG_TIMED_BLOCK(
                poll, DEVEL, 5.0, { LOG_DEBUG_MSG("poll_recv_queue"); });
            ...
            if (ret > 0)
            {
                LOG_DEBUG_MSG("Completion rxcq wr_id "
                    << fi_tostr(&entry.flags, FI_TYPE_OP_FLAGS) << " ("
                    << decnumber(entry.flags) << ") "
                    << "source " << hexpointer(src_addr) << "context "
                    << hexpointer(entry.op_context) << "length "
                    << hexuint32(entry.len));
            ...
            else if (ret == -FI_EAVAIL)
            {
                ...
                LOG_ERROR_MSG("rxcq Error with flags "
                    << hexlength(e.flags) << "len " << hexlength(e.len));
            }
            else
            {
        ...
        int poll_event_queue(bool /*stopped*/ = false)
        {
            ...
            ssize_t rd = fi_eq_read(
                event_queue_, &event, buffer.data(), sizeof(buffer), 0);
            if (rd > 0)
            {
                LOG_DEBUG_MSG("fi_eq_cm_entry "
                    << decnumber(sizeof(fi_eq_cm_entry)) << " fi_eq_entry "
                    << decnumber(sizeof(fi_eq_entry)));
                LOG_DEBUG_MSG(
                ...
                {
                    cm_entry =
                        reinterpret_cast<struct fi_eq_cm_entry*>(buffer.data());
                        ...
                        << ipaddress(here_.ip_address()) << " )");
                    {
                        ...
                        if (!result.first &&
                            loc.ip_address() > here_.ip_address())
                        {
                            ...
                            //     nullptr, 0);
                            // if (ret) {
                            //     throw fabric_error(ret, "new_ep fi_reject failed");
                            // }
                        ...
                        if (ret)
                            throw fabric_error(ret, "new_ep fi_accept failed");
                    }
                    fi_freeinfo(cm_entry->info);
                    break;
                }
                case FI_CONNECTED:
                {
                    ...
                    {
                        throw fabric_error(
                            0, "FI_CONNECTED, endpoint map error");
                    }
                    LOG_DEBUG_MSG("FI_CONNECTED "
                        << hexpointer(new_ep)
            ...
            {
                LOG_TIMED_MSG(
                    poll, DEVEL, 5, "We did not get an event completion")
            }
            return 0;
        }
        ...
        void create_completion_queues(struct fi_info* info, int N)
        {
            ...
            LOG_DEBUG_MSG(
                "Creating CQ with tx size " << decnumber(info->tx_attr->size));
            ...
            LOG_DEBUG_MSG(
                "Creating CQ with rx size " << decnumber(info->rx_attr->size));
            ...
            uint32_t remote_ip)
        {
            ...
            LOG_DEBUG_MSG("Inserting future in map         "
                << ipaddress(here_.ip_address()) << "-> "
                << ipaddress(remote_ip) << "( " << ipaddress(here_.ip_address())
                << " )");
            ...
            //
            auto fp_pair = std::make_pair(remote_ip,
                std::make_tuple(HPX_MOVE(new_endpoint_promise),
                    HPX_MOVE(new_endpoint_future)));
            //
            auto it = endpoint_tmp_.insert(HPX_MOVE(fp_pair));
        ...
        fi_addr_t insert_address(const locality& remote)
        {
            ...
            int ret =
                fi_av_insert(av_, remote.fabric_data(), 1, &result, 0, nullptr);
            if (ret < 0)
            {
            ...
            LOG_DEBUG_MSG("Address inserted in vector "
                << ipaddress(remote.ip_address()) << ":" << remote.port()
                << hexuint64(result));
            FUNC_END_DEBUG_MSG;
            return result;
        }
        ...
        // --------------------------------------------------------------------
        hpx::shared_future<struct fid_ep*> connect_to_server(
            const locality& remote)
        {
            ...
            std::memcpy(new_info->dest_addr, remote.fabric_data(),
                locality::array_size);

            uint64_t flags = 0;
            struct fi_info* fabric_info_active_;
            int ret = fi_getinfo(FI_VERSION(1, 4), nullptr, nullptr, flags,
                new_info, &fabric_info_active_);
            if (ret)
                throw fabric_error(ret, "fi_getinfo");

            LOG_DEBUG_MSG("New connection for IP address "
                << ipaddress(remote.ip_address()) << "Fabric info "
                << fi_tostr(fabric_info_active_, FI_TYPE_INFO));
            ...
            if (ret)
                throw fabric_error(ret, "fi_connect");
