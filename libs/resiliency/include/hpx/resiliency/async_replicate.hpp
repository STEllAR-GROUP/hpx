...
//  Copyright (c) 2018-2019 Adrain Serio
...
namespace hpx { namespace resiliency {
    ...
    /// against a user provided voting function. Return the valid output.
        ...
        return hpx::dataflow(
            ...
            [pred = std::forward<Pred>(pred), vote = std::forward<Vote>(vote),
                ...
                if (!valid_results.empty())
                    return vote(std::move(valid_results));
                if (ex != nullptr)
    ...
    async_replicate_validate(std::size_t n, Pred&& pred, F&& f, Ts&&... ts)
    {
        ...
        return async_replicate_vote_validate(n, vote_<result_type>,
    ...
    ///////////////////////////////////////////////////////////////////////////
    /// Asynchronously launch given function \a f exactly \a n times. Verify the result of
    /// those invocations by checking for exception. Return the first
    /// valid result.
