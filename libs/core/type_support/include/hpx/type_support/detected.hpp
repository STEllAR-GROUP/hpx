...
namespace hpx::util {
    ...
    HPX_CXX_CORE_EXPORT struct nonesuch
    {
        nonesuch() = delete;
        ~nonesuch() = delete;
    ...
    // std::false_type.
    HPX_CXX_CORE_EXPORT template <template <typename...> class Op,
        typename... Args>
    using is_detected =
    ...
    //   for Default.
    HPX_CXX_CORE_EXPORT template <typename Default,
        template <typename...> class Op, typename... Args>
    ...
    HPX_CXX_CORE_EXPORT template <typename Expected,
        template <typename...> class Op, typename... Args>
    using is_detected_exact = std::is_same<Expected, detected_t<Op, Args...>>;

    // The alias template is_detected_convertible checks whether
    // detected_t<Op, Args...> is convertible to To.
    HPX_CXX_CORE_EXPORT template <typename To, template <typename...> class Op,
        typename... Args>
