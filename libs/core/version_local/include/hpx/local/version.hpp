...
namespace hpx::local {
    ...
    HPX_LOCAL_EXPORT std::uint8_t major_version();

    // Returns the minor HPX version.
    HPX_LOCAL_EXPORT std::uint8_t minor_version();

    // Returns the sub-minor/patch-level HPX version.
    HPX_LOCAL_EXPORT std::uint8_t subminor_version();

    // Returns the full HPX version.
    HPX_LOCAL_EXPORT std::uint32_t full_version();
    ...
    HPX_LOCAL_EXPORT std::string full_build_string();

    // Returns the HPX version string.
    HPX_LOCAL_EXPORT std::string build_string();

    // Returns the Boost version string.
    HPX_LOCAL_EXPORT std::string boost_version();

    // Returns the Boost platform string.
    HPX_LOCAL_EXPORT std::string boost_platform();

    // Returns the Boost compiler string.
    HPX_LOCAL_EXPORT std::string boost_compiler();

    // Returns the Boost standard library string.
    HPX_LOCAL_EXPORT std::string boost_stdlib();

    // Returns the copyright string.
    HPX_LOCAL_EXPORT std::string copyright();

    // Returns the full version string.
    HPX_LOCAL_EXPORT std::string complete_version();

    // Returns the HPX build type ('Debug', 'Release', etc.)
    HPX_LOCAL_EXPORT std::string build_type();

    // Returns the HPX build date and time
    HPX_LOCAL_EXPORT std::string build_date_time();

    // Return the HPX configuration information
    HPX_LOCAL_EXPORT std::string configuration_string();
}    // namespace hpx::local
