...
struct test_receiver {
    ...
    friend void tag_invoke(ex::set_value_t, test_receiver& r) noexcept {
        ...
        r.completed = true;
        if (r.bulk_count > 0) r.bulk_count = 3;  // Mark bulk completion
    }

    friend void tag_invoke(ex::set_error_t, test_receiver&&, std::exception_ptr) noexcept {
...
int hpx_main(hpx::program_options::variables_map&) {
    ...
    std::cout << "Single task completed: " << (recv.completed ? "true" : "false") << std::endl;
    ...
    auto bulk_op = ex::connect(ex::bulk(ex::schedule(sched), 3, [](int i) {
        std::cout << "Bulk functor called for index: " << i << std::endl;
    }), recv);
