...
namespace hpx { namespace functional {
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Helper base class implementing the tag_invoke logic for CPOs that call
    // user-defined overloads only if the override overloads do not match.
    template <typename Tag>
    struct tag_override
    {
        // is override tag-invocable
        ...
        // is not override tag-invocable, but tag-invocable
        template <typename... Args,
            typename Enable = typename std::enable_if<
                !is_tag_override_invocable_v<Tag, Args&&...>>::type>
        constexpr HPX_FORCEINLINE auto operator()(Args&&... args) const
    ...
    // Helper base class implementing the tag_invoke logic for CPOs that call
    // user-defined overloads only if the override overloads do not match.
    // Either invocation has to be noexcept.
    template <typename Tag>
    struct tag_override_noexcept
    {
    private:
        ...
        template <typename... Args>
        constexpr HPX_FORCEINLINE auto tag_override_invoke_impl(
    ...
    public:
        // is override nothrow tag-invocable
        template <typename... Args,
            typename IsOverrideInvocable =
                is_nothrow_tag_override_invocable<Tag, Args&&...>>
        constexpr HPX_FORCEINLINE auto operator()(Args&&... args) const noexcept
            -> decltype(tag_override_invoke_impl(
                IsOverrideInvocable{}, std::forward<Args>(args)...))
        {
            return tag_override_invoke_impl(
                IsOverrideInvocable{}, std::forward<Args>(args)...);
        }

        // is not override nothrow tag-invocable, but tag-invocable
        template <typename... Args,
            typename Enable = typename std::enable_if<
