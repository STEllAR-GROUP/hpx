...
{
    {
        ...
        std::unordered_multimap<T, A<T>> os;
        for (T c = min; c < max; ++c)
        {
        ...
        std::unordered_multimap<T, A<T>> is;
        iarchive >> is;
...
void test_vector_as_value()
{
    std::vector<char> buffer;
    hpx::serialization::output_archive oarchive(buffer);
    std::unordered_multimap<size_t, std::vector<int>> os;
    for (int k = 0; k < 10; ++k)
    {
    ...
    oarchive << os;
    hpx::serialization::input_archive iarchive(buffer);
    std::unordered_multimap<size_t, std::vector<int>> is;
    iarchive >> is;
    HPX_TEST_EQ(os.size(), is.size());
    for (auto const& v : os)
    {
