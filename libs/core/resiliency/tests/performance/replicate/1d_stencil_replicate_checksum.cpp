...
struct stepper
{
        ...
        partition_data const& right_input)
    {
        ...
        double left_checksum = std::accumulate(
            end(left_input) - static_cast<std::ptrdiff_t>(sti) - 1,
            end(left_input), 0.0);
        double right_checksum = std::accumulate(begin(right_input),
            begin(right_input) + static_cast<std::ptrdiff_t>(sti) + 1, 0.0);
        ...
        std::shared_ptr<hpx::sliding_semaphore> sem)
    {
        ...
        using hpx::resiliency::experimental::dataflow_replicate_validate;
        ...
        for (std::size_t t = 0; t != iterations; ++t)
        {
            ...
            {
                next[i] = dataflow_replicate_validate(n_value, validate_result,
                    Op, sti, error, current[(i - 1 + subdomains) % subdomains],
