...
namespace hpx::threads::policies {
    ...
    scheduler_base::scheduler_base(std::size_t num_threads,
      ...
      , wait_mtxs_(num_threads)
      , wait_conds_(num_threads)
#endif
    ...
    void scheduler_base::idle_callback([[maybe_unused]] std::size_t num_thread)
    {
            ...
            policies::scheduler_mode::enable_idle_backoff)
        {
            ...
            std::unique_lock<pu_mutex_type> l(wait_mtxs_[num_thread]);
            if (wait_conds_[num_thread].wait_for(l, period) ==    //-V1089
    ...
    void scheduler_base::do_some_work([[maybe_unused]] std::size_t num_thread)
    {
            ...
            policies::scheduler_mode::enable_idle_backoff)
        {
            ...
            {
                auto const size = wait_conds_.size();
                for (std::size_t i = 0; i != size; ++i)
                {
                    wait_counts_[i].data_.wait_count_ = 0;
                    wait_conds_[i].notify_all();
                }
            }
            else
            {
                wait_counts_[num_thread].data_.wait_count_ = 0;
                wait_conds_[num_thread].notify_all();
            }
#endif
        }
    }
