...
void read_composite()
{
    ...
    second.push(compose(toupper_filter<input>(),
        compose(padding_filter('a'), file_source(src1.name(), in_mode))));
    ...
    first.push(padding_filter('a'));
    first.push(file_source(src1.name(), in_mode));
    second.push(compose(compose(toupper_filter<input>(), padding_filter('a')),
...
void write_composite()
{
    ...
    // Test composite device
    out1.push(tolower_filter<output>());
    out1.push(padding_filter('a'));
    out1.push(file_sink(dest1.name(), in_mode));
    out2.push(compose(tolower_filter<output>(),
        compose(padding_filter('a'), file_sink(dest2.name(), in_mode))));
    ...
    out1.push(padding_filter('a'));
    out1.push(file_sink(dest1.name(), in_mode));
    out2.push(compose(compose(tolower_filter<output>(), padding_filter('a')),
...
void close_composite_device()
{
    // Compose an input filter with a source
    {
        ...
        chain<input> ch;
        ch.push(io::compose(closable_filter<input>(seq.new_operation(2)),
    ...
    // Compose a bidirectional filter with a source
    {
        operation_sequence seq;
        chain<input> ch;
        ch.push(io::compose(closable_filter<bidirectional>(
    ...
    // Compose a seekable filter with a source
    {
        operation_sequence seq;
        chain<input> ch;
        ch.push(io::compose(closable_filter<seekable>(seq.new_operation(2)),
            closable_device<input>(seq.new_operation(1))));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
    ...
    // Compose a dual-use filter with a source
    {
        ...
        operation dummy;
        ch.push(
            io::compose(closable_filter<dual_use>(seq.new_operation(2), dummy),
                closable_device<input>(seq.new_operation(1))));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
    ...
    // Compose an output filter with a sink
    {
        operation_sequence seq;
        chain<output> ch;
        ch.push(io::compose(closable_filter<output>(seq.new_operation(1)),
    ...
    // Compose a seekable filter with a sink
    {
        operation_sequence seq;
        chain<output> ch;
        ch.push(io::compose(closable_filter<seekable>(seq.new_operation(1)),
            closable_device<output>(seq.new_operation(2))));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
    ...
    // Compose a dual-use filter with a sink
    {
        ...
        operation dummy;
        ch.push(
            io::compose(closable_filter<dual_use>(dummy, seq.new_operation(1)),
                closable_device<output>(seq.new_operation(2))));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
...
void close_composite_filter()
{
    // Compose two input filters
    {
        operation_sequence seq;
        chain<input> ch;
        ch.push(io::compose(closable_filter<input>(seq.new_operation(3)),
    ...
    // Compose a seekable filter with an input filter
    {
        operation_sequence seq;
        chain<input> ch;
        ch.push(io::compose(closable_filter<seekable>(seq.new_operation(3)),
            closable_filter<input>(seq.new_operation(2))));
        ch.push(closable_device<input>(seq.new_operation(1)));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    ...
    // Compose a dual-use filter with an input filter
    {
        ...
        operation dummy;
        ch.push(
            io::compose(closable_filter<dual_use>(seq.new_operation(3), dummy),
                closable_filter<input>(seq.new_operation(2))));
        ch.push(closable_device<input>(seq.new_operation(1)));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    ...
    // Compose two bidirectional filters
    {
        ...
        ch.push(io::compose(closable_filter<bidirectional>(
            ...
            closable_filter<bidirectional>(
                seq.new_operation(2), seq.new_operation(5))));
    ...
    // Compose two dual-use filters for input
    {
        ...
        ch.push(
            io::compose(closable_filter<dual_use>(seq.new_operation(3), dummy),
                closable_filter<dual_use>(seq.new_operation(2), dummy)));
        ch.push(closable_device<input>(seq.new_operation(1)));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
    ...
    // Compose two dual-use filters for output
    {
        ...
        ch.push(
            io::compose(closable_filter<dual_use>(dummy, seq.new_operation(1)),
                closable_filter<dual_use>(dummy, seq.new_operation(2))));
        ch.push(closable_device<output>(seq.new_operation(3)));
        HPX_TEST_NO_THROW(ch.reset());
        check_operation_sequence(seq);
    }
