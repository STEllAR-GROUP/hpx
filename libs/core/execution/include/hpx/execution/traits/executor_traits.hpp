...
namespace hpx { namespace parallel { namespace execution {
            ...
            struct has_sync_execute_member
              : detail::has_sync_execute<std::decay_t<T>>
            {
            };

            template <typename T, typename Enable = void>
            struct has_async_execute_member
              : detail::has_async_execute<std::decay_t<T>>
            {
            };

            template <typename T, typename Enable = void>
            struct has_then_execute_member
              : detail::has_then_execute<std::decay_t<T>>
            {
            };
            ...
            public:
                using type = hpx::util::detected_or_t<
                    typename executor_shape<Executor>::type, index_type,
                    Executor>;
            };
            ...
            public:
                using type =
                    hpx::util::detected_or_t<hpx::execution::static_chunk_size,
            ...
            namespace detail {
                ...
                {
                    using type =
                        decltype(std::declval<Executor&&>().async_execute(
                            std::declval<T (*)(Ts...)>(),
                            std::declval<Ts>()...));
                };
