...
namespace hpx::execution::experimental {
    ...
    struct wrapped_params : detail::wrapped_params_properties<Wrapped, Wrapping>
    {
        ...
        template <typename Params, typename Executor, typename F>
            requires(std::same_as<wrapped_params, std::decay_t<Params>> && (
                supports_wrapping_measure_iteration_v<
                    detail::wrapping_t<Params>,
        ...
        // clang-format on
        friend constexpr void tag_override_invoke(
            collect_execution_parameters_t, Params&& this_, Executor&& exec,
            std::size_t const num_elements, std::size_t const num_cores,
            std::size_t const num_chunks, std::size_t const chunk_size)
        {
        ...
        tag_override_invoke(adjust_chunk_size_and_max_chunks_t, Params&& this_,
            Executor&& exec, std::size_t const num_elements,
