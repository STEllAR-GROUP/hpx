...
template <typename F>
auto tag_invoke(ex::upon_stopped_t, custom_sender_tag_invoke s, F&&)
{
...
{
    {
        ...
        std::atomic<bool> upon_stopped_callback_called{false};
        auto s1 = stopped_sender{};
        auto s2 = ex::upon_stopped(std::move(s1), [&]() {
            upon_stopped_callback_called = true;
            return 42;
        });
        ...
        HPX_TEST(upon_stopped_callback_called);
    }
    ...
    {
        std::atomic<bool> set_value_called{false};
        std::atomic<bool> upon_stopped_callback_not_called{true};
        auto s1 = ex::just(42);
        auto s2 = ex::upon_stopped(std::move(s1), [&]() {
            upon_stopped_callback_not_called = false;
            return 0;
        });
        ...
        ex::start(os);
        HPX_TEST(set_value_called);
        HPX_TEST(upon_stopped_callback_not_called);
    }
