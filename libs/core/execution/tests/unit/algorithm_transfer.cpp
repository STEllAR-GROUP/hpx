...
struct scheduler_schedule_from : example_scheduler_template<scheduler_schedule_from>
{
...
struct sender_with_completion_scheduler : void_sender
{
    ...
    struct my_env {
        ...
        friend scheduler_transfer const& tag_invoke(
            ex::get_completion_scheduler_t<ex::set_value_t>, my_env env) noexcept
        {
            return env.sched;
        }
    };

    friend my_env tag_invoke(
        hpx::execution::experimental::get_env_t, sender_with_completion_scheduler const& s) noexcept
    {
        return {s.sched};
    }
#else
...
int main()
{
    // Success path
    {
        ...
        auto s = ex::transfer(ex::just(),
            example_scheduler{scheduler_schedule_called, scheduler_execute_called,
    ...
    // tag_invoke overloads
    {
        ...
        static_assert(ex::scheduler<scheduler_schedule_from>);
