...
{    // {{{
    ...
    {
        HPX_THROW_EXCEPTION(
            no_success, "topology::topology", "Failed to init hwloc topology");
    }
    ...
    {
        HPX_THROW_EXCEPTION(
            no_success, "topology::topology", "Failed to load hwloc topology");
    }
...
{    // {{{
...

    {
        ...
        {
            std::string errstr = hpx::util::strerror(errno);
    ...
    const void* addr, std::size_t len, void* nodeset) const
{
    ...
    if (ret < 0)
    {
        std::string msg = hpx::util::strerror(errno);
        if (errno == ENOSYS)
...
int topology::get_numa_domain(const void* addr) const
{
    ...
    int ret =
        hwloc_get_area_memlocation(topo, addr, 1, ns, HWLOC_MEMBIND_BYNODESET);
    if (ret < 0)
    {
