...
friend typename hpx::parallel::util::detail::algorithm_result<ExPolicy, T>::type
tag_fallback_invoke(
    hpx::reduce_t, ExPolicy&& policy, FwdIter first, FwdIter last, T init)
{
    static_assert(hpx::traits::is_forward_iterator<FwdIter>::value,
        "Requires at least forward iterator.");
    ...
    hpx::reduce_t, InIter first, InIter last, T init, F&& f)
{
    ...
    return hpx::parallel::v1::detail::reduce<T>().call(
        hpx::execution::seq, first, last, HPX_MOVE(init), HPX_FORWARD(F, f));
}
...
// clang-format on
friend T tag_fallback_invoke(hpx::reduce_t, InIter first, InIter last, T init)
{
    static_assert(hpx::traits::is_input_iterator<InIter>::value,
        "Requires at least input iterator.");

    return hpx::parallel::v1::detail::reduce<T>().call(
        hpx::execution::seq, first, last, HPX_MOVE(init), std::plus<>{});
}
...
// clang-format on
friend typename std::iterator_traits<InIter>::value_type tag_fallback_invoke(
    hpx::reduce_t, InIter first, InIter last)
{
    static_assert(hpx::traits::is_input_iterator<InIter>::value,
        "Requires at least input iterator.");
