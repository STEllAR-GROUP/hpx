...
#include "../algorithms/none_of_tests.hpp"
...
void test_none_of()
{
    ...
    test_none_of(simd, IteratorTag());
    test_none_of(par_simd, IteratorTag());

    test_none_of_async(simd(task), IteratorTag());
    test_none_of_async(par_simd(task), IteratorTag());
}
...
void test_none_of_exception()
{
    ...
    test_none_of_exception(simd, IteratorTag());
    test_none_of_exception(par_simd, IteratorTag());

    test_none_of_exception_async(simd(task), IteratorTag());
    test_none_of_exception_async(par_simd(task), IteratorTag());
}
...
void test_none_of_bad_alloc()
{
    ...
    // std::terminate shall be called. therefore we do not test exceptions
    // with a vector execution policy
    test_none_of_bad_alloc(simd, IteratorTag());
    test_none_of_bad_alloc(par_simd, IteratorTag());

    test_none_of_bad_alloc_async(simd(task), IteratorTag());
    test_none_of_bad_alloc_async(par_simd(task), IteratorTag());
}
