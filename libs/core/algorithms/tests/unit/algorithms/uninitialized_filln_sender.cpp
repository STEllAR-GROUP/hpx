    ...
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    ...
    tt::sync_wait(ex::just(iterator(std::begin(c)), c.size(),
                      std::size_t(10)) |
...
void uninitialized_fill_n_sender_test()
{
    ...
    test_uninitialized_fill_n_sender(hpx::launch::sync, seq(task), IteratorTag());
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    uninitialized_fill_n_exception_sender_test<std::random_access_iterator_tag>();

    uninitialized_fill_n_bad_alloc_sender_test<std::forward_iterator_tag>();
    uninitialized_fill_n_bad_alloc_sender_test<std::random_access_iterator_tag>();
