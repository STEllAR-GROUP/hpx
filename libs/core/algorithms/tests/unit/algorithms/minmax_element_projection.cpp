...
void test_min_element_projection()
{
    ...
    auto proj = [](const element& p) { return p.second; };

    // Test sequential
    auto r_seq = hpx::min_element(seq, c.begin(), c.end(), std::less<int>{}, proj);
    HPX_TEST(r_seq != c.end());
    HPX_TEST_EQ(r_seq->second, 5);

    // Test parallel
    auto r_par = hpx::min_element(par, c.begin(), c.end(), std::less<int>{}, proj);
...
void test_max_element_projection()
{
    ...
    // Test sequential
    auto r_seq = hpx::max_element(seq, c.begin(), c.end(), std::less<int>{}, proj);
    HPX_TEST(r_seq != c.end());
    HPX_TEST_EQ(r_seq->second, 15);

    // Test parallel
    auto r_par = hpx::max_element(par, c.begin(), c.end(), std::less<int>{}, proj);
    HPX_TEST(r_par != c.end());
...
void test_minmax_element_projection()
{
    ...
    // Test sequential
    auto r_seq = hpx::minmax_element(seq, c.begin(), c.end(), std::less<int>{}, proj);
    ...
    // Test parallel
    auto r_par = hpx::minmax_element(par, c.begin(), c.end(), std::less<int>{}, proj);
