...
void test_partition_sender(LnPolicy ln_policy, ExPolicy&& ex_policy,
    IteratorTag)
{
    ...
    const std::size_t size = 30007;
    const int rand_base = _gen();

    random_fill gen_functor{rand_base, size / 10};
    auto pred = [rand_base](const int n) -> bool { return n < rand_base; };

    std::vector<int> c(size), c_org;
    std::generate(std::begin(c), std::end(c), gen_functor);
    c_org = c;
    ...
    auto snd_result = tt::sync_wait(
        ...
        | hpx::partition(ex_policy.on(exec))
    );
