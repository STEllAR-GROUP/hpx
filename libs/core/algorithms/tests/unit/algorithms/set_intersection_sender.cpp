...
void test_set_intersection_sender(
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    ...
    auto result =
        tt::sync_wait(ex::just(iterator(std::begin(c1)), iterator(std::end(c1)),
            ...
            hpx::set_intersection(ex_policy.on(exec)));

    auto const expected_last = std::set_intersection(std::begin(c1),
...
// sender pipeline
template <typename LnPolicy, typename ExPolicy, typename IteratorTag>
void test_set_intersection_sender_comp(
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    static_assert(hpx::is_async_execution_policy_v<ExPolicy>,
        "hpx::is_async_execution_policy_v<ExPolicy>");
...
// degenerate ranges -- an empty input on either side yields an empty
// intersection; these flow through set_operation's sentinel-initialized
// set_chunk_data instead of an early return on the S/R path
template <typename LnPolicy, typename ExPolicy, typename IteratorTag>
void test_set_intersection_sender_empty(
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    static_assert(hpx::is_async_execution_policy_v<ExPolicy>,
        "hpx::is_async_execution_policy_v<ExPolicy>");
    ...
    // empty second range -> empty result, dest unchanged
    {
        std::vector<std::size_t> c1 = test::random_fill(10007);
        std::sort(std::begin(c1), std::end(c1));
        std::vector<std::size_t> c2;
        std::vector<std::size_t> c3(c1.size(), static_cast<std::size_t>(-1));
...
void set_intersection_sender_test()
{
    using namespace hpx::execution;
    test_set_intersection_sender(hpx::launch::sync, seq(task), IteratorTag());
    test_set_intersection_sender(hpx::launch::sync, unseq(task), IteratorTag());
    test_set_intersection_sender(hpx::launch::async, par(task), IteratorTag());
    test_set_intersection_sender(
        hpx::launch::async, par_unseq(task), IteratorTag());

    test_set_intersection_sender_comp(
    ...
    test_set_intersection_sender_empty(
        hpx::launch::sync, seq(task), IteratorTag());
    test_set_intersection_sender_empty(
        hpx::launch::sync, unseq(task), IteratorTag());
    test_set_intersection_sender_empty(
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    set_intersection_sender_test<std::forward_iterator_tag>();
    set_intersection_sender_test<std::random_access_iterator_tag>();
