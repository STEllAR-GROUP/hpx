    ...
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    ...
    // union output can be as large as both inputs combined
    ...
    auto const expected_last = std::set_union(std::begin(c1), std::end(c1),
...
// sender pipeline
template <typename LnPolicy, typename ExPolicy, typename IteratorTag>
void test_set_union_sender_comp(
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    static_assert(hpx::is_async_execution_policy_v<ExPolicy>,
        "hpx::is_async_execution_policy_v<ExPolicy>");
...
// degenerate ranges -- the union with an empty range is the other range. On the
// S/R path the early exits are skipped, so these exercise whether set_operation
// reproduces the non-empty side on its own.
template <typename LnPolicy, typename ExPolicy, typename IteratorTag>
void test_set_union_sender_empty(
    LnPolicy ln_policy, ExPolicy&& ex_policy, IteratorTag)
{
    static_assert(hpx::is_async_execution_policy_v<ExPolicy>,
        "hpx::is_async_execution_policy_v<ExPolicy>");
    ...
    // empty first range -> result is a copy of the second range
    {
    ...
    // empty second range -> result is a copy of the first range
    {
        std::vector<std::size_t> c1 = test::random_fill(10007);
        std::sort(std::begin(c1), std::end(c1));
...
void set_union_sender_test()
{
    ...
    test_set_union_sender_comp(hpx::launch::sync, seq(task), IteratorTag());
    test_set_union_sender_comp(hpx::launch::sync, unseq(task), IteratorTag());
    test_set_union_sender_comp(hpx::launch::async, par(task), IteratorTag());
    test_set_union_sender_comp(
        hpx::launch::async, par_unseq(task), IteratorTag());

    test_set_union_sender_empty(hpx::launch::sync, seq(task), IteratorTag());
    test_set_union_sender_empty(hpx::launch::sync, unseq(task), IteratorTag());
    test_set_union_sender_empty(hpx::launch::async, par(task), IteratorTag());
    test_set_union_sender_empty(
        hpx::launch::async, par_unseq(task), IteratorTag());
}
