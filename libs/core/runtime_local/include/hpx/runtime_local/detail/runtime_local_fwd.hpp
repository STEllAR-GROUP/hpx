...
namespace hpx {
    ...
    HPX_LOCAL_EXPORT runtime& get_runtime();
    HPX_LOCAL_EXPORT runtime*& get_runtime_ptr();

    /// Return true if networking is enabled.
    ///
    /// \note Networking is enabled if `-DHPXLocal_WITH_NETWORKING=On` was used at
    ///       configuration time and more than one locality is used or the
    ///       command line option `--hpx:expect-connecting-localities` was
    ///       specified
    HPX_LOCAL_EXPORT bool is_networking_enabled();
}    // namespace hpx
