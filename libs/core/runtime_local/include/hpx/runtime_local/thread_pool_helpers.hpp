...
namespace hpx::resource {
    ...
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::size_t get_num_thread_pools();

    /// Return the number of threads in all thread pools currently
    /// managed by the \a resource_partitioner
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::size_t get_num_threads();
    ...
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::size_t get_pool_index(
        std::string const& pool_name);

    /// Return the name of the pool given its internal index
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::string const& get_pool_name(
        std::size_t pool_index);

    /// Return the name of the pool given its name
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT threads::thread_pool_base&
    get_thread_pool(std::string const& pool_name);

    /// Return the thread pool given its internal index
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT threads::thread_pool_base&
    get_thread_pool(std::size_t pool_index);

    /// Return true if the pool with the given name exists
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT bool pool_exists(
        std::string const& pool_name);
...
namespace hpx::threads {
    ...
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::int64_t get_thread_count(
    ...
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT std::int64_t get_idle_core_count();

    /// The function \a get_idle_core_mask returns a bit-mask representing the
    /// currently idling threads (cores).
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT mask_type get_idle_core_mask();
    ...
    HPX_CXX_CORE_EXPORT HPX_CORE_EXPORT bool enumerate_threads(
