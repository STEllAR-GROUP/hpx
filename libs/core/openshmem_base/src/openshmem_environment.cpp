...
namespace hpx::util {
    ...
    int openshmem_environment::init(int* argc, char*** argv, int& provided)
    {
        ...
        {
            HPX_THROW_EXCEPTION(error::invalid_status,
                "hpx::util::openshmem_environment::init", "OPENSHMEM resource error");
        }
        else if (openshmem_environment::init_val_ == 0)
        {
            HPX_THROW_EXCEPTION(error::invalid_status,
        ...
        hpx::util::openshmem_environment::shmem_buffer = static_cast<std::uint8_t*>(
            shmem_calloc(OPENSHMEM_PER_RANK_PAGESIZE, sizeof(std::uint8_t))
        );

        for(int i = 0; i < size(); ++i) {
            segments[i].addr = hpx::util::openshmem_environment::shmem_buffer+(i*OPENSHMEM_PER_RANK_PAGESIZE);
            segments[i].size = static_cast<std::uint8_t*>(segments[i].addr)+OPENSHMEM_PER_RANK_PAGESIZE;
        }
        ...
        int* argc, char*** argv, util::runtime_configuration& rtcfg)
    {
        ...
        int retval =
            init(argc, argv, provided_threading_flag_);
        if (1 != retval)
        {
            ...
            std::string msg("openshmem_environment::init: openshmem_init failed");
    ...
    void openshmem_environment::put_signal(const std::uint8_t* addr, const int node,
        std::uint8_t* raddr, const std::size_t size, unsigned int * sigaddr)
    {
        if (rank() == node)
        {
           ...
           shmem_uint8_put_signal(
              raddr,
              addr,
              size, reinterpret_cast<std::uint64_t*>(sigaddr), 1,
              SHMEM_SIGNAL_SET, node);
        }
    }

    void openshmem_environment::wait_until(const unsigned int value, unsigned int * sigaddr) {
        ...
        const std::uint8_t* raddr, const std::size_t size)
    {
        ...
        {
            shmem_uint8_get(addr, raddr, size, node);    // dest, node, src, size
        }
    }

    void openshmem_environment::global_barrier() { shmem_barrier_all(); }
