...
namespace hpx { namespace ranges {
    ...
    /// Remarks: If \a f returns a result, the result is ignored.
    ///
    template <typename Rng, typename Size, typename... Args>
    void for_loop_n(Rng&& rng, Size size, Args&&... args);
    ...
    ///           otherwise.
    ///
    template <typename ExPolicy, typename Rng, typename Size, typename... Args>
    typename util::detail::algorithm_result<ExPolicy>::type for_loop_n(
        ExPolicy&& policy, Rng&& rng, Size size, Args&&... args);

    /// The for_loop_n_strided implements loop functionality over a range specified by
    /// a range. These algorithms resemble for_each from the
    /// Parallelism TS, but leave to the programmer when and if to dereference
    /// the iterator.
    ///
    /// The execution of for_loop_n_strided without specifying an execution policy is
    /// equivalent to specifying \a hpx::execution::seq as the execution
    /// policy.
    ///
    /// \tparam Rng         The type of the source range used (deduced).
    ...
    ///
    template <typename Rng, typename Size, typename... Args>
    void for_loop_n_strided(Rng&& rng, Size size, Args&&... args);
...
namespace hpx { namespace ranges {
    ...
    private:
        ...
        // clang-format on
        friend typename parallel::util::detail::algorithm_result<ExPolicy>::type
        tag_invoke(hpx::ranges::for_loop_n_t, ExPolicy&& policy, Rng&& rng,
            Size size, Args&&... args)
        {
            static_assert(sizeof...(Args) >= 1,
            ...
            return parallel::v2::detail::for_loop_n(
                std::forward<ExPolicy>(policy), hpx::util::begin(rng), size, 1,
                typename make_index_pack<sizeof...(Args) - 1>::type(),
                std::forward<Args>(args)...);
        }
        ...
        // clang-format on
        friend void tag_invoke(
            hpx::ranges::for_loop_n_t, Rng&& rng, Size size, Args&&... args)
        {
            static_assert(sizeof...(Args) >= 1,
                "for_loop_n must be called with at least a function object");

            using hpx::util::make_index_pack;
            return parallel::v2::detail::for_loop_n(hpx::execution::seq,
                hpx::util::begin(rng), size, 1,
                typename make_index_pack<sizeof...(Args) - 1>::type(),
                std::forward<Args>(args)...);
        }
    } for_loop_n{};
    ...
    private:
        // clang-format off
        template <typename ExPolicy, typename Rng, typename Size, typename S,
            typename... Args,
            HPX_CONCEPT_REQUIRES_(
                hpx::is_execution_policy<ExPolicy>::value &&
                std::is_integral<Size>::value &&
        ...
        // clang-format on
        friend typename parallel::util::detail::algorithm_result<ExPolicy>::type
        tag_invoke(hpx::ranges::for_loop_n_strided_t, ExPolicy&& policy,
            Rng&& rng, Size size, S stride, Args&&... args)
        {
            static_assert(sizeof...(Args) >= 1,
            ...
            using hpx::util::make_index_pack;
            return parallel::v2::detail::for_loop_n(
                std::forward<ExPolicy>(policy), hpx::util::begin(rng), size,
                stride, typename make_index_pack<sizeof...(Args) - 1>::type(),
                std::forward<Args>(args)...);
        }

        // clang-format off
        template <typename Rng, typename Size, typename S, typename... Args,
            HPX_CONCEPT_REQUIRES_(
                std::is_integral<Size>::value &&
                std::is_integral<S>::value &&
                hpx::traits::is_range<Rng>::value
        ...
        // clang-format on
        friend void tag_invoke(hpx::ranges::for_loop_n_strided_t, Rng&& rng,
            Size size, S stride, Args&&... args)
        {
            ...
            using hpx::util::make_index_pack;
            return parallel::v2::detail::for_loop_n(hpx::execution::seq,
                hpx::util::begin(rng), size, stride,
                typename make_index_pack<sizeof...(Args) - 1>::type(),
                std::forward<Args>(args)...);
        }
    } for_loop_n_strided{};
