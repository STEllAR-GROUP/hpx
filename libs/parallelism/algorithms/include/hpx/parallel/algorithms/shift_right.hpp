...
shift_right_helper(ExPolicy policy, FwdIter first, FwdIter new_first,
                   FwdIter last) {
  ...
  return dataflow(
      ...
      -> hpx::future<util::in_out_result<FwdIter, FwdIter>> {
        ...
        hpx::future<FwdIter> f = r.call2(p, non_seq(), last, first);
        return f.then([=](hpx::future<FwdIter> &&f) mutable
                      -> util::in_out_result<FwdIter, FwdIter> {
          f.get(); // propagate exceptions
          std::advance(std::distance(new_first, last), first);
          return util::in_out_result<FwdIter, FwdIter>{last, first};
        });
      },
...
struct shift_right : public detail::algorithm<shift_right<IterPair>, IterPair> {
  shift_right() : shift_right::algorithm("shift_right") {}

  template <typename ExPolicy, typename InIter>
  static IterPair sequential(ExPolicy, InIter first, InIter new_first,
                             InIter last) {
    return detail::sequential_rotate(last, new_first, first);
  }

  template <typename ExPolicy, typename FwdIter>
  static typename util::detail::algorithm_result<ExPolicy, IterPair>::type
  parallel(ExPolicy &&policy, FwdIter first, FwdIter new_first, FwdIter last) {
    return util::detail::algorithm_result<ExPolicy, IterPair>::get(
        shift_right_helper(std::forward<ExPolicy>(policy), last, new_first,
                           first));
  }
};
/// \endcond
...
/// Performs a right rotation on a range of elements. Specifically,
/// \a shift_right swaps the elements in the range [first, last) in such a way
...
///
/// The assignments in the parallel \a shift_right algorithm invoked
/// with an execution policy object of type \a sequenced_policy
/// execute in sequential order in the calling thread.
///
/// The assignments in the parallel \a shift_right algorithm invoked with
...
///           otherwise.
///           The \a shift_right algorithm returns the iterator equal to
