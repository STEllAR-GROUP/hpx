...
shift_left_helper(ExPolicy policy, FwdIter first, FwdIter new_first,
                  FwdIter last) {
  ...
  return dataflow(
      ...
      -> hpx::future<util::in_out_result<FwdIter, FwdIter>> {
        ...
        return f.then([=](hpx::future<FwdIter> &&f) mutable
          ...
          return util::in_out_result<FwdIter, FwdIter>{first, last};
        });
      },
...
struct shift_left : public detail::algorithm<shift_left<IterPair>, IterPair> {
  shift_left() : shift_left::algorithm("shift_left") {}
  ...
  parallel(ExPolicy &&policy, FwdIter first, FwdIter new_first, FwdIter last) {
    return util::detail::algorithm_result<ExPolicy, IterPair>::get(
        shift_left_helper(std::forward<ExPolicy>(policy), first, new_first,
                          last));
  }
};
/// \endcond
