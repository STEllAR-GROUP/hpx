...
#ifndef OCLM_AND_SELECTOR_HPP
#define OCLM_AND_SELECTOR_HPP
...
namespace oclm
{
    ...
    template <typename S1, typename S2>
    struct is_platform_selector<detail::and_selector<S1, S2> >
        : boost::mpl::or_<
    ...
    template <typename S1, typename S2>
    struct is_device_selector<detail::and_selector<S1, S2> >
        : boost::mpl::or_<
    ...
    typename boost::enable_if<
        ...
        >
      , selector<detail::and_selector<F1, F2> >
    >::type
    operator&&(F1 const & s1, F2 const & s2)
    {
        detail::and_selector<F1, F2> sel = {s1, s2};
        return selector<detail::and_selector<F1, F2> >(sel);
    }
}
