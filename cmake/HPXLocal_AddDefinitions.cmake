...
function(hpx_local_add_config_define definition)
  ...
  if(ArgsLen GREATER 0)
    set_property(
      GLOBAL APPEND PROPERTY HPXLocal_CONFIG_DEFINITIONS
                             "${definition} ${ARGN}"
    )
  else()
    set_property(
      GLOBAL APPEND PROPERTY HPXLocal_CONFIG_DEFINITIONS "${definition}"
    )
  endif()

endfunction()

function(hpx_local_add_config_cond_define definition)
  ...
  if(ArgsLen GREATER 0)
    set_property(
      GLOBAL APPEND PROPERTY HPXLocal_CONFIG_COND_DEFINITIONS
                             "${definition} ${ARGN}"
    )
  else()
    set_property(
      GLOBAL APPEND PROPERTY HPXLocal_CONFIG_COND_DEFINITIONS "${definition}"
    )
  endif()
...
# ---------------------------------------------------------------------
function(hpx_local_add_config_define_namespace)
  set(options)
  ...
  set(DEF_VAR HPXLocal_CONFIG_DEFINITIONS_${OPTION_NAMESPACE})
...
function(hpx_local_add_config_cond_define_namespace)
  set(one_value_args DEFINE NAMESPACE)
  set(multi_value_args VALUE)
  cmake_parse_arguments(
    OPTION "${options}" "${one_value_args}" "${multi_value_args}" ${ARGN}
  ...
  set(DEF_VAR HPXLocal_CONFIG_COND_DEFINITIONS_${OPTION_NAMESPACE})
...
# ---------------------------------------------------------------------
function(hpx_local_write_config_defines_file)
  set(options)
  ...
  if(${OPTION_NAMESPACE} STREQUAL "default")
    get_property(DEFINITIONS_VAR GLOBAL PROPERTY HPXLocal_CONFIG_DEFINITIONS)
    get_property(
      COND_DEFINITIONS_VAR GLOBAL PROPERTY HPXLocal_CONFIG_COND_DEFINITIONS
    )
  else()
    get_property(
      DEFINITIONS_VAR GLOBAL
      PROPERTY HPXLocal_CONFIG_DEFINITIONS_${OPTION_NAMESPACE}
    )
    get_property(
      COND_DEFINITIONS_VAR GLOBAL
      PROPERTY HPXLocal_CONFIG_COND_DEFINITIONS_${OPTION_NAMESPACE}
    )
  endif()
  ...
  foreach(def ${COND_DEFINITIONS_VAR})
    ...
    if(NOT ${_pos} EQUAL -1)
      set(hpx_local_config_defines
          "${hpx_local_config_defines}#if __cplusplus >= 202002 && !defined(${defname})\n#define ${def}\n#endif\n"
      )
    else()
      ...
      if(NOT ${_pos} EQUAL -1)
        set(hpx_local_config_defines
            "${hpx_local_config_defines}#if __cplusplus >= 201500 && !defined(${defname})\n#define ${def}\n#endif\n"
        )
      else()
        set(hpx_local_config_defines
            "${hpx_local_config_defines}#if !defined(${defname})\n#define ${def}\n#endif\n"
  ...
  if(NOT OPTION_TEMPLATE)
    ...
    file(WRITE ${TEMP_FILENAME} ${PREAMBLE} ${hpx_local_config_defines} "\n")
