  ...
  if(${name}_DISABLE)
    ...
    if(NOT ${name}_SEARCHED)
      ...
      if(${name}_FOUND)
        ...
        set(${name}_ROOT ${${name}_ROOT} CACHE PATH "${name} root directory." ADVANCED)
        set(${name}_LIBRARY ${${name}_LIBRARY} CACHE FILEPATH "${name} shared library." ADVANCED)
        set(${name}_LIBRARY_DIR ${${name}_LIBRARY_DIR} CACHE PATH "${name} library directory." ADVANCED)
        set(${name}_INCLUDE_DIR ${${name}_INCLUDE_DIR} CACHE PATH "${name} include directory." ADVANCED)
      else()
        set(${name}_FOUND OFF CACHE BOOL "Found ${name}.")
        set(${name}_LIBRARY ${name}_LIBRARY-NOTFOUND CACHE FILEPATH "${name} shared library." ADVANCED)
        set(${name}_LIBRARY_DIR ${name}_LIBRARY_DIR-NOTFOUND CACHE PATH "${name} library directory." ADVANCED)
        set(${name}_INCLUDE_DIR ${name}_INCLUDE_DIR-NOTFOUND CACHE PATH "${name} include directory." ADVANCED)
      endif()
