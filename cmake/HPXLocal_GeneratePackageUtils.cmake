...
function(
  ...
  hpx_local_get_target_property(
    _target_link_libraries ${target} INTERFACE_LINK_LIBRARIES
  )
  hpx_local_get_target_property(
    _target_link_options ${target} INTERFACE_LINK_OPTIONS
  )
  hpx_local_get_target_property(_target_type ${target} TYPE)

  if(NOT "${_target_type}" STREQUAL "INTERFACE_LIBRARY")
    hpx_local_get_target_property(
      _target_imported_location ${target} IMPORTED_LOCATION
    )
    ...
    foreach(dep IN LISTS _target_link_libraries)
      ...
      if(${dep} MATCHES "^\\$<LINK_ONLY:([^>]+)>$")
        ...
        if(TARGET ${dep_target})
          hpx_local_get_target_property(dep_type ${dep_target} TYPE)
...
endfunction(hpx_local_collect_usage_requirements)

# Isolate the build properties from the install ones
function(hpx_local_sanitize_usage_requirements property is_build)
...
endfunction(hpx_local_sanitize_usage_requirements)

function(hpx_local_filter_cuda_flags cflag_list)
...
endfunction(hpx_local_filter_cuda_flags)
...
endfunction(hpx_local_construct_cflag_list)

function(hpx_local_construct_library_list link_libraries link_options
         library_list
)
...
endfunction(hpx_local_construct_library_list)

# Configure the corresponding package config template for the specified
# ${template}
function(hpx_local_generate_pkgconfig_from_target target template is_build)
  ...
  hpx_local_sanitize_usage_requirements(
    hpx_local_compile_definitions ${is_build}
  )
  hpx_local_sanitize_usage_requirements(hpx_local_compile_options ${is_build})
  hpx_local_sanitize_usage_requirements(
    hpx_local_include_directories ${is_build}
  )
  hpx_local_sanitize_usage_requirements(
    hpx_local_system_include_directories ${is_build}
  )
  hpx_local_sanitize_usage_requirements(hpx_link_libraries ${is_build})
  hpx_local_sanitize_usage_requirements(hpx_link_options ${is_build})
...
endfunction(hpx_local_generate_pkgconfig_from_target)
